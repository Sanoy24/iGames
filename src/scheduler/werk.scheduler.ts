...
export class WerkScheduler implements OnApplicationShutdown {
    ...
    fast(): void {
        ...
        try {
            ...
            this.logger.error(
                `Werk fast tick failed: ${err instanceof Error ? err.message : err}`,
            );
        }
    }
    ...
    async lifecycle(): Promise<void> {
        ...
        try {
            ...
            this.logger.error(
                `Werk lifecycle failed: ${err instanceof Error ? err.message : err}`,
            );
        } finally {
    ...
    private async renewLeadership(): Promise<boolean> {
        if (this.heldLock) {
            const extend = (
                this.heldLock as unknown as {
                    extend?: (ttl: number) => Promise<Lock>;
                }
            ).extend;
            if (typeof extend === 'function') {
                try {
                    this.heldLock = await extend.call(
                        this.heldLock,
                        WERK_LEADER_LOCK_TTL_MS,
                    );
                    this.leaderUntil =
                        Date.now() + WERK_LEADER_LOCK_TTL_MS - 500;
                    return true;
                } catch {
