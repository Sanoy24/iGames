...
export function initScriptFromCommands(commands) {
    const body = commands
        .map(
            (command) => `echo "=== ${escapeForEcho(command)} ==="\n${command}`,
        )
        .join('\n\n');
    return `#!/bin/bash
set -e
...
export function scoreHarness(files) {
    ...
    const featureList =
        byPath.get('feature_list.json') ||
        byPath.get('feature-list.json') ||
        '';
    ...
    const checks = {
        instructions: [
            ...
            structuredHas(
                agents,
                ['Definition of Done', 'done only when'],
    ...
    const subsystems = Object.fromEntries(
        Object.entries(checks).map(([name, subsystemChecks]) => {
            ...
            const score = Math.max(
                1,
                Math.round((passed / subsystemChecks.length) * 5),
            );
            return [
                name,
                {
    ...
    const total = Object.values(subsystems).reduce(
        (sum, item) => sum + item.score,
        0,
    );
    const overall = Math.round((total / (SUBSYSTEMS.length * 5)) * 100);
    const ranked = Object.entries(subsystems).sort(
...
function textHas(text, needles, message) {
    ...
    return {
        pass: needles.some((needle) => lower.includes(needle.toLowerCase())),
        message,
    };
}
...
export function formatScoreReport(result, root = '.') {
    ...
    for (const [name, subsystem] of Object.entries(result.subsystems)) {
        lines.push(
            `${name}: ${subsystem.score}/5 (${subsystem.passed}/${subsystem.total})`,
        );
