...
export function WerkFelega({ onBack }: { onBack: () => void }) {
    ...
    const adoptParticipation = useCallback((r: WerkRoundView) => {
        if (r.yourParticipantId && r.yourSeat != null) {
            setMyPart({
                roundId: r.id,
                participantId: r.yourParticipantId,
                seat: r.yourSeat,
            });
        }
    }, []);
    ...
    const startPlaying = useCallback(
        (v: WerkRoundView, seat: number | null) => {
            if (!config) return;
            const g = new WerkGame({
                ...
                botCount: v.botCount,
                coinDensityX100: v.coinDensityX100,
                finalSprintWarningSec: v.finalSprintWarningSec,
                powerupsEnabled: v.powerupsEnabled,
                theme: (v.mazeTheme ?? config.mazeTheme) as MazeTheme,
                yourSeat: seat,
                yourName: user?.displayName ?? 'You',
            });
            g.onCollect = (type) => {
                if (type === 'gold') goldPickup();
                else coinPickup();
            };
        ...
        (v: WerkRoundView) => {
            ...
            const mine =
                v.standings.find(
                    (s) =>
                        s.participantId &&
                        s.participantId === mp?.participantId,
                ) ??
            ...
            if (eliminated) lossThud();
            else if (rank <= 3 && prizeMinor > 0) victoryFanfare();
            ...
            if (prizeMinor > 0)
                confetti({
                    ...
                    spread: 85,
                    origin: { y: 0.6 },
                    colors: [C.success, C.coin, C.accent],
                });
            walletApi
                .getWallet()
                .then(setWallet)
    ...
    useWerkSocket(roundId, {
        ...
        onSnapshot: (s) => {
            gameRef.current?.applySnapshot(s);
        },
        onCompleted: (v) => {
            handleCompleted(v);
        },
    ...
    const spectate = () => {
        if (round && round.status !== 'none') startPlaying(round, null);
    };
    ...
    if (screen === 'lobby') {
        ...
        const iAmInLobby =
            ...
            !!round &&
            !!myPart &&
            myPart.roundId === round.id;
        return (
            <div
                style={{
                    minHeight: '70vh',
                    background: `radial-gradient(1200px 600px at 50% -10%, rgba(124,92,255,0.14), transparent), radial-gradient(900px 500px at 50% 120%, rgba(0,212,255,0.10), transparent), ${C.bg}`,
                    borderRadius: 20,
            ...
            >
                ...
                >
                    <div
                        style={{
                            ...
                            color: C.coin,
                            textShadow: `0 0 30px rgba(255,213,74,0.45)`,
                            lineHeight: 1.1,
                        }}
                    >
                        ወርቅ ፍለጋ
                ...
                {inProgress && !iAmInLobby && (
                    ...
                    >
                        ...
                        >
                            ...
                            />
                            {round?.playerCount ?? 0} ·{' '}
                            {Math.ceil(round?.timeLeft ?? 0)}s {t('werk.left')}
                        </div>
                        <button
                            onClick={spectate}
                            style={{
                ...
                {iAmInLobby && (
                    ...
                    >
                        ...
                        >
                            {round?.countdown != null ? round.countdown : '…'}
                        </div>
                        <div
                            style={{
                                fontSize: 12,
                    ...
                    <div style={{ ...glass, padding: 18, marginBottom: 14 }}>
                        ...
                        >
                            <Stat
                                label={t('werk.mode')}
                                value={
                                    config?.winningMode === 'B'
                                        ? t('werk.modeB')
                                        : t('werk.modeA')
                                }
                            />
                            <Stat
                                label={t('werk.players')}
                                value={`${round?.playerCount ?? 0}/${config?.totalPlayers ?? '–'}`}
                            />
                            <Stat
                                label={t('werk.duration')}
                                value={`${config?.gameDurationSec ?? '–'}s`}
                            />
                        </div>
                        <div
                            style={{
                            ...
                            {mults.map(
                                (m, i) =>
                                    m > 0 && (
                                        ...
                                        >
                                            #{i + 1} ·{' '}
                                            {Math.floor((stake * m) / 100)}
                                        </span>
                                    ),
                            )}
                        </div>
                        {(round?.playerCount ?? 0) > 0 && (
                        ...
                        >
                            ...
                            <button
                                ...
                                style={{
                                    ...primaryBtn,
                                    flex: 1,
                                    opacity: busy || !config?.enabled ? 0.6 : 1,
                                }}
                            >
                                <Play size={17} />{' '}
                                {busy ? '…' : t('werk.joinRound')}
                            </button>
                        </div>
                    </div>
                )}
    ...
    return (
        ...
        >
            {g && (
                <TopHud
                    game={g}
                    name={user?.displayName ?? t('werk.you')}
                    muted={muted}
            ...
            {g && spectating && (
                ...
                >
                    ...
                    <div style={{ flex: 1, fontSize: 12.5, color: C.dim }}>
                        {t('werk.joinNextHint')}
                    </div>
                    <button
                        onClick={leaveGame}
                    ...
                    style={{
                        ...
                        background:
                            p.userId === youId
                                ? 'rgba(0,212,255,0.14)'
                                : 'rgba(30,36,45,0.7)',
                        border: `1px solid ${p.userId === youId ? 'rgba(0,212,255,0.4)' : C.border}`,
                        fontSize: 12,
                    }}
                >
                    <span
    ...
    return (
        ...
        >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <button onClick={onLeave} style={iconBtn} aria-label='Leave'>
                    <ArrowLeft size={16} />
                </button>
                <div
                    style={{
                ...
                >
                    ...
                    >
                        {spectating ? <Eye size={16} /> : initial}
                    </div>
                    <div style={{ minWidth: 0 }}>
                        ...
                        >
                            {spectating ? t('werk.spectating') : name}
                        </div>
                        <div
                            style={{
                                fontSize: 10,
                        ...
                        >
                            {mine ? (
                                ...
                                <>
                                    {game.players.length} {t('werk.players')}
                                </>
                            )}
                        </div>
                    </div>
                ...
                <div
                    style={{
                        ...
                        background: isSprint
                            ? 'rgba(255,92,92,0.85)'
                            : 'rgba(30,36,45,0.7)',
                        border: `1px solid ${isSprint ? C.danger : C.border}`,
                    }}
                >
                    <span
                        style={{
                ...
                <button onClick={onMute} style={iconBtn} aria-label='Mute'>
            ...
            {h && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    >
                        <span style={{ color: COIN_COLOR.bronze }}>
                            ●{h.bronze}
                        </span>
                        <span style={{ color: COIN_COLOR.silver }}>
                            ●{h.silver}
                        </span>
                        <span style={{ color: COIN_COLOR.gold }}>
                            ●{h.gold}
                        </span>
                    </span>
                    <div
                    ...
                    >
                        <div
                            style={{
                                ...
                                height: '100%',
                                background:
                                    h.stamina > 30 ? C.success : C.danger,
...
function ControlDeck({
    ...
    game: WerkGame;
    inputRef: React.MutableRefObject<HumanInput>;
}) {
    const { t } = useTranslation();
    const h = game.human!;
                        ...
                        {top.map((s, i) => (
                            <span
                                ...
                                style={{
                                    ...
                                    borderRadius: 999,
                                    flexShrink: 0,
                                    background: s.player.isHuman
                                        ? 'rgba(0,212,255,0.14)'
                                        : 'rgba(30,36,45,0.7)',
                                    border: `1px solid ${s.player.isHuman ? 'rgba(0,212,255,0.4)' : C.border}`,
                                }}
                            >
                                ...
                                <span
                                    style={{
                                        fontSize: 11,
                                        fontWeight: s.player.isHuman
                                            ? 800
                                            : 600,
                                        color: s.player.isHuman
                                            ? C.accent
                                            : C.text,
                                        maxWidth: 64,
                                        overflow: 'hidden',
                                ...
                                >
                                    {s.player.isHuman &&
                                    s.player.id >= 1_000_000 &&
                                    s.player.name === game.human?.name
                                        ? t('werk.you')
                                        : s.player.name}
                                </span>
                                <span
            ...
            >
                ...
                >
                    <button
                        onPointerDown={(e) => {
                            e.preventDefault();
                            inputRef.current.usePower = true;
                        }}
                        ...
                        style={{
                            ...
                            border: `2px solid ${powerReady ? C.coin : 'rgba(255,213,74,0.4)'}`,
                            color: C.coin,
                            fontSize: 24,
                            touchAction: 'none',
                            boxShadow: powerReady
...
function Joystick({
    ...
    const track = (clientX: number, clientY: number) => {
        ...
        const mag =
            raw <= JOY_DEADZONE ? 0 : (raw - JOY_DEADZONE) / (1 - JOY_DEADZONE);
    ...
    return (
        <div
            ...
            style={{
                ...
                cursor: 'pointer',
                background:
                    'radial-gradient(circle at 50% 38%, rgba(40,48,60,0.92), rgba(18,22,28,0.92))',
        ...
        >
            ...
            <div
                ...
                style={{
                    ...
                    pointerEvents: 'none',
                    background:
                        'radial-gradient(circle at 40% 34%, rgba(0,212,255,0.95), rgba(0,140,190,0.9))',
                    border: '2px solid rgba(0,212,255,0.7)',
                    boxShadow:
                        '0 4px 16px rgba(0,0,0,0.55), 0 0 14px rgba(0,212,255,0.35)',
                }}
            />
        </div>
    );
...
function Minimap({ game, size = 118 }: { game: WerkGame; size?: number }) {
    ...
    useEffect(() => {
        ...
        for (const c of game.coins)
            if (!c.collected)
                ctx.fillRect(c.x * scale - 0.5, c.y * scale - 0.5, 1.3, 1.3);
...
function ResultScreen({
    ...
    useEffect(() => {
        ...
        const tick = (now: number) => {
            ...
            setShownPrize(
                Math.round(result.prizeMinor * (1 - Math.pow(1 - p, 3))),
            );
    ...
    return (
        <div
            style={{
                minHeight: '80vh',
                background: `radial-gradient(1000px 500px at 50% -10%, rgba(124,92,255,0.16), transparent), ${C.bg}`,
                borderRadius: 20,
        ...
        >
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
                ...
                <div style={{ color: C.dim, fontSize: 14 }}>
                    {t('werk.finishedRank', { rank: result.rank })}
                </div>
                {result.prizeMinor > 0 && (
                    <div
                        style={{
                            ...
                            fontWeight: 900,
                            color: C.coin,
                            textShadow: '0 0 26px rgba(255,213,74,0.5)',
                        }}
                    >
                        +{shownPrize}
                    </div>
            ...
            >
                {[1, 0, 2].map((idx) => {
                    const s = podium[idx];
                    if (!s) return <div key={idx} style={{ width: 86 }} />;
                    const heights = [102, 76, 58];
                    const grad =
                        idx === 1
                            ? 'linear-gradient(#FFD54A,#c79b16)'
                            : idx === 0
                              ? 'linear-gradient(#cfd6df,#8a8f99)'
                              : 'linear-gradient(#cd7f32,#8a5320)';
                    return (
                        <div
                            key={idx}
                        ...
                        >
                            <div
                                style={{
                                    ...
                                    textOverflow: 'ellipsis',
                                    whiteSpace: 'nowrap',
                                    color: s.isHuman ? C.accent : C.text,
                                }}
                            >
                                {s.isHuman ? s.name : s.name}
                            </div>
                            <div
                                style={{
                                    height: heights[idx],
                ...
                {result.standings.map((s) => (
                    <div
                        ...
                        style={{
                            ...
                            padding: '4px',
                            fontSize: 13,
                            fontWeight: s.isHuman ? 800 : 500,
                            color: s.isHuman ? C.accent : C.text,
                            opacity: s.eligible ? 1 : 0.45,
                        }}
                    >
                        <span
                            style={{
...
> = {
    adwa: {
        floor: '#0e131a',
        tile: 'rgba(124,92,255,0.05)',
        tileEdge: 'rgba(0,212,255,0.10)',
        wall: '#C79B36',
        wallGlow: 'rgba(212,160,23,0.4)',
        hub: '#FCDD09',
    },
    highland: {
        floor: '#0b1613',
        tile: 'rgba(52,211,153,0.05)',
        tileEdge: 'rgba(52,211,153,0.14)',
        wall: '#2FBF8F',
        wallGlow: 'rgba(47,191,143,0.4)',
        hub: '#34D399',
    },
    desert: {
        floor: '#191308',
        tile: 'rgba(251,191,36,0.06)',
        tileEdge: 'rgba(251,191,36,0.15)',
        wall: '#E0A23A',
        wallGlow: 'rgba(224,162,58,0.4)',
        hub: '#FBBF24',
    },
};
...
function drawWorld(
    ...
    const focus = game.human ?? {
        x: game.center[0] * CELL + CELL / 2,
        y: game.center[1] * CELL + CELL / 2,
    };
    let camX = focus.x - vw / 2,
        camY = focus.y - vh / 2;
    ...
    {
        const tsc = Math.max(0, Math.floor(camX / CELL) - 1),
            tec = Math.min(game.size - 1, Math.ceil((camX + vw) / CELL) + 1);
        const tsr = Math.max(0, Math.floor(camY / CELL) - 1),
            ter = Math.min(game.size - 1, Math.ceil((camY + vh) / CELL) + 1);
        ctx.fillStyle = th.tile;
        ctx.strokeStyle = th.tileEdge;
        ctx.lineWidth = 1;
        for (let gy = tsr; gy <= ter; gy++)
            for (let gx = tsc; gx <= tec; gx++) {
                roundRectPath(
                    ctx,
                    gx * CELL + 3,
                    gy * CELL + 3,
    ...
    const hubX = ccx * CELL + CELL / 2,
        hubY = ccy * CELL + CELL / 2;
    ...
    grad.addColorStop(
        0,
        isSprint
            ? `rgba(255,92,92,${0.6 + pulse * 0.4})`
            : `rgba(255,213,74,${0.5 + pulse * 0.35})`,
    );
    ...
    const sc = Math.max(0, Math.floor(camX / CELL) - 1),
        ec = Math.min(game.size - 1, Math.ceil((camX + vw) / CELL) + 1);
    const sr = Math.max(0, Math.floor(camY / CELL) - 1),
        er = Math.min(game.size - 1, Math.ceil((camY + vh) / CELL) + 1);
    ...
    for (let gy = sr; gy <= er; gy++)
        for (let gx = sc; gx <= ec; gx++) {
            ...
            if (gx === game.size - 1 && cell.right) {
                ctx.moveTo(x1, y0);
                ctx.lineTo(x1, y1);
            }
            if (gy === game.size - 1 && cell.bottom) {
                ctx.moveTo(x0, y1);
                ctx.lineTo(x1, y1);
            }
        }
