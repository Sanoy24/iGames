...
const tiers = (w1: number, w2: number, w34: number, semis: string[] = ['c', 'd'], runnerUp = 'b') => [
  { place: 1, weight: w1, players: ['a'] },
  { place: 2, weight: w2, players: runnerUp ? [runnerUp] : [] },
  { place: 3, weight: w34, players: semis },
];

const total = (rows: Array<{ amountMinor: number }>) => rows.reduce((s, r) => s + r.amountMinor, 0);
...
describe('computeTournamentPayouts', () => {
  ...
  it('splits across 1st/2nd/3rd-4th by weight and equalises the tied tier', () => {
    ...
    const map = Object.fromEntries(out.map((r) => [r.userId, r.amountMinor]));
