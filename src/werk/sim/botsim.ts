...
export class BotSim {
    ...
    constructor(
        ...
        for (const c of layout.coins)
            this.coinAtCell.set(c.cy * layout.size + c.cx, c.index);
        ...
        this.bots = roster.map((r, i) => {
            ...
            return {
                ...
                skill: r.skill,
                speed: HUMAN_SPEED * (r.speedPct / 100),
                x,
                y,
                coinValue: 0,
                reachedCenter: false,
    ...
    private isTaken(index: number): boolean {
        return this.shared
            ? this.shared.has(index)
            : this.botTaken[index] === 1;
    }
    ...
    private decide(b: BotRuntime) {
        ...
        const headHome =
            this.cfg.mode === 'B' &&
            ((b.personality === 'strategist' && this.timeLeft < 30) ||
                this.timeLeft <= this.cfg.finalSprintWarningSec + 3);
        if (headHome) {
            b.headingHome = true;
        ...
        for (const c of coins) {
            ...
            candidates.push({
                idx: c.index,
                md: Math.abs(c.cx - bcx) + Math.abs(c.cy - bcy),
            });
        }
        ...
        for (const { idx, md } of pool) {
            ...
            if (b.personality === 'chaotic')
                score += (this.rng() - 0.5) * 20 * (1.3 - b.skill);
            if (score > bestScore) {
                bestScore = score;
                best = idx;
            }
        ...
        if (pool.length && this.rng() > 0.35 + b.skill * 0.6)
            best = pool[Math.floor(this.rng() * pool.length)].idx;
        if (best < 0) return;
    ...
    finish() {
        ...
        for (const b of this.bots) {
            ...
            b.reachedCenter =
                cx === this.layout.center[0] && cy === this.layout.center[1];
        }
    }
}
...
export function computeStandings(
    ...
    const entries: StandingRow[] = [
        {
            ...
            isHuman: true,
            color: human.color,
            coinValue: human.coinValue,
            eligible: eligibleOf(human.reachedCenter),
            rank: 0,
        },
        ...bots.map(
            (b): StandingRow => ({
                ...
                color: b.color,
                coinValue: b.coinValue,
                eligible: eligibleOf(b.reachedCenter),
                rank: 0,
            }),
        ),
    ];
    ...
    const humanTieCount = mine.eligible
        ? entries.filter((e) => e.eligible && e.rank === mine.rank).length
        : 1;
    return {
        rows: entries,
        humanRank: mine.rank,
        humanTieCount,
        humanEligible: mine.eligible,
    };
}
