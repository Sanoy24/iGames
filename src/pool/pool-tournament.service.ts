...
export class PoolTournamentService {
  ...
  private async completeTournament(t: PoolTournament, finalMatch: PoolMatch): Promise<void> {
    const winnerId = winnerUserIdOf(finalMatch);
    if (!winnerId) return;
    const runnerUpId = finalMatch.winnerSeat === 'A' ? finalMatch.seatBUserId : finalMatch.seatAUserId;

    // Semi-final losers (places 3–4) — only exist for brackets of 4+.
    const semiLoserIds: string[] = [];
    if (t.rounds >= 2) {
      const semis = await this.matchRepo.find({
        where: { tournamentId: t.id, tournamentRound: t.rounds - 2, status: 'completed' },
      });
      for (const s of semis) {
        const loser = s.winnerSeat === 'A' ? s.seatBUserId : s.seatAUserId;
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      const pool = Math.floor((fresh.prizePoolMinor * (100 - fresh.rakePct)) / 100);
      const tiers = [
        { place: 1, weight: cfg.tournamentPrize1Weight ?? 100, players: [winnerId] },
        { place: 2, weight: cfg.tournamentPrize2Weight ?? 0, players: runnerUpId ? [runnerUpId] : [] },
        { place: 3, weight: cfg.tournamentPrize34Weight ?? 0, players: semiLoserIds },
      ];
        ...
        await this.walletService.creditInSession(
          {
            ...
            metadata: { prizePool: fresh.prizePoolMinor, rakePct: fresh.rakePct, place },
          },
          manager,
        );
      }
...
export function computeTournamentPayouts(
  ...
  const active = tiers.filter((tier) => tier.weight > 0 && tier.players.length > 0);
  ...
  if (pool <= 0 || totalWeight <= 0) {
    ...
    return winner && pool > 0 ? [{ userId: winner, amountMinor: pool, place: 1 }] : [];
  }
  let distributed = 0;
