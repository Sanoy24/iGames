...
export class CrashService {
    ...
    private async broadcastBetPlaced(bet: CrashBetResponse): Promise<void> {
        try {
            ...
            this.logger.debug(
                `Crash bet broadcast skipped: ${err instanceof Error ? err.message : err}`,
            );
        }
    }
    ...
    private broadcastCashout(bet: CrashBetResponse): void {
        try {
            ...
            this.logger.debug(
                `Crash cashout broadcast skipped: ${err instanceof Error ? err.message : err}`,
            );
        }
    }
    ...
    async updateConfig(
        dto: Partial<Omit<CrashConfig, 'key' | 'createdAt' | 'updatedAt'>>,
    ): Promise<CrashConfig> {
        let cfg = await this.configRepo.findOneBy({ key: 'default' });
    ...
    async abandonStaleRounds(): Promise<number> {
        ...
        const candidates = await this.roundRepo.find({
            where: { status: Not('crashed') as any },
        });
        const stale = candidates.filter((round) => {
            if (round.status === 'waiting')
                return now - round.createdAt.getTime() > MAX_WAITING_AGE_MS;
            if (round.status === 'running')
                return (
                    !round.startedAt ||
                    now - round.startedAt.getTime() > MAX_RUNNING_AGE_MS
                );
            return false;
        });
        ...
        for (const round of stale) {
            await this.dataSource.transaction(async (manager) => {
                ...
                const fresh = await manager.findOne(CrashRound, {
                    where: { id: round.id },
                });
                if (fresh && fresh.status !== 'crashed') {
                    fresh.status = 'crashed';
                    fresh.crashedAt = new Date();
                    if (fresh.crashPointX100 == null)
                        fresh.crashPointX100 = 100;
        ...
        this.logger.warn(
            `Abandoned ${stale.length} stale crash round(s) on startup`,
        );
        return stale.length;
    }
    ...
    async createRound(): Promise<CrashRoundResponse> {
        ...
        this.logger.log(
            `Created Crash round ${saved.id} (hash: ${seedHash.slice(0, 12)}…)`,
        );
    ...
    async startRound(
        roundId: string,
    ): Promise<{ round: CrashRoundResponse; crashPointX100: number }> {
        return this.dataSource.transaction(async (manager) => {
            ...
            if (round.status !== 'waiting')
                throw new ConflictException(`Round is ${round.status}`);
            if (!round.seed)
                throw new Error(
                    `Round ${roundId} has no seed  cannot generate crash point`,
                );

            const cfg = await this.getConfig();
            const crashPointX100 = await this.generateCrashPoint(
                round.id,
                round.seed,
                cfg.houseEdgePct,
                cfg.maxMultiplierX100,
                manager,
            );
            ...
            this.logger.log(
                `Crash round ${round.id} started  crash at ${(crashPointX100 / 100).toFixed(2)}×`,
            );
    ...
    async settleRound(
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            const wonBets = await manager.find(CrashBet, {
                where: { roundId, status: 'won' },
            });
    ...
    async placeBet(
        ...
        if (dto.stakeMinor < cfg.minBetMinor) {
            throw new BadRequestException(
                `Minimum bet is ${cfg.minBetMinor} ETB`,
            );
        }
        if (dto.stakeMinor > cfg.maxBetMinor) {
            throw new BadRequestException(
                `Maximum bet is ${cfg.maxBetMinor} ETB`,
            );
        }
    ...
    async cashOut(
        ...
        const result = await this.dataSource.transaction(async (manager) => {
            ...
            if (currentMultiplierX100 >= (round.crashPointX100 ?? 0)) {
                throw new BadRequestException(
                    'Cannot cash out  round has already crashed',
                );
            }

            const payoutMinor = Math.floor(
                (bet.stakeMinor * currentMultiplierX100) / 100,
            );
    ...
    async processAutoCashouts(
        ...
        const toProcess = eligible.filter(
            (b) =>
                b.autoCashoutX100 !== null &&
                currentMultiplierX100 >= b.autoCashoutX100,
        );
        ...
        for (const bet of toProcess) {
            try {
                ...
                this.logger.warn(
                    `Auto-cashout failed for bet ${bet.id}: ${err instanceof Error ? err.message : err}`,
                );
            }
        }
        return results;
    ...
    private toRoundResponse(
        ...
        return {
            ...
            crashPointX100:
                round.status === 'crashed' ? round.crashPointX100 : null,
