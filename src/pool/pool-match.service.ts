...
export class PoolMatchService {
    ...
    private physicsFor(match: PoolMatch): {
        table: TableSpec;
        tuning: ShotTuning;
    } {
        const snap = snapshotFromConfig(
            (match.physics ?? {}) as Partial<PoolPhysicsSnapshot>,
        );
    ...
    async createMatch(
        ...
        const repo = manager
            ? manager.getRepository(PoolMatch)
            : this.matchRepo;
    ...
    private deadlineFrom(cfg: PoolConfig): Date | null {
        return cfg.shotClockSeconds > 0
            ? new Date(Date.now() + cfg.shotClockSeconds * 1000)
            : null;
    }
    ...
    private validateInput(match: PoolMatch, input: ShotInput): void {
        ...
        if (!finite(input.angle) || !finite(input.power)) {
            throw new BadRequestException(
                'angle and power must be finite numbers',
            );
        }
        ...
        if (
            !input.spin ||
            !finite(input.spin.side) ||
            !finite(input.spin.vertical)
        ) {
            throw new BadRequestException(
                'spin.side and spin.vertical are required numbers',
            );
        }
        if (
            Math.abs(input.spin.side) > 1 ||
            Math.abs(input.spin.vertical) > 1
        ) {
        ...
        if (input.cuePos) {
            if (!match.ballInHand) {
                throw new BadRequestException(
                    'Cue placement is only allowed with ball in hand',
                );
            }
            ...
            if (
                ...
                throw new BadRequestException(
                    'Cue placement is outside the table',
                );
            }
            const clash = match.board.some(
                (b) =>
                    b.number !== 0 &&
                    !b.pocketed &&
                    Math.hypot(b.pos.x - x, b.pos.y - y) < 2 * R,
            );
            if (clash)
                throw new BadRequestException(
                    'Cue placement overlaps another ball',
                );
        }
    }
    ...
    async submitShot(
        ...
        const { outcome, match: after } = await this.applyResolvedShot(
            matchId,
            seat,
            input,
        );
    ...
    private botSeatOf(match: PoolMatch): Seat | null {
        ...
        return match.seatBUserId === null
            ? 'B'
            : match.seatAUserId === null
              ? 'A'
              : null;
    }
    ...
    private async maybeRunBotTurns(match: PoolMatch): Promise<void> {
        ...
        if (botSeat && match.turn === botSeat)
            await this.runBotTurns(match.id, botSeat);
    }
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                ...
                if (outcome.gameOver && outcome.winner) {
                    const settled = await this.settleWithinTx(
                        manager,
                        match,
                        outcome.winner,
                        cfg,
                ...
                const res = await manager.getRepository(PoolMatch).update(
                    { id: match.id, shotCount: shotIndex },
                    {
                        ...
                        settledAt,
                        turnDeadline: outcome.gameOver
                            ? null
                            : this.deadlineFrom(cfg),
                    },
                );
                if (!res.affected)
    ...
    private async runBotTurns(matchId: string, botSeat: Seat): Promise<void> {
        ...
        for (let i = 0; i < MAX_BOT_SHOTS_PER_TURN; i++) {
            ...
            const { table } = this.physicsFor(match);
            const input = await this.botService.computeShot(
                this.toState(match),
                botSeat,
                cfg.botDifficulty,
                table,
    ...
    private async settleWithinTx(
        ...
        const winnerUserId =
            winner === 'A' ? match.seatAUserId : match.seatBUserId;
    ...
    async handleShotTimeout(matchId: string): Promise<void> {
        const match = await this.getMatch(matchId);
        if (match.status !== 'active') return;
        if (!match.turnDeadline || match.turnDeadline.getTime() > Date.now())
            return;
        ...
        const cfg = await this.poolService.getConfig();
        const prior =
            (offender === 'A' ? match.timeoutsA : match.timeoutsB) ?? 0;
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            const countGuard =
                offender === 'A' ? { timeoutsA: prior } : { timeoutsB: prior };
            ...
            if (forfeit) {
                ...
                if (!res.affected)
                    throw new ConflictException('Match already advanced');
            } else {
                ...
                if (!res.affected)
                    throw new ConflictException('Timeout already applied');
            }
        });
        ...
        if (forfeit) {
            ...
            await this.maybeRunBotTurns(fresh).catch((e) =>
                this.logger.warn(
                    `Bot turn after timeout failed for ${matchId}: ${e?.message ?? e}`,
                ),
            );
        }
    }
    ...
    private broadcastMatch(m: PoolMatch): void {
        try {
            ...
            this.logger.debug(
                `match broadcast skipped: ${e instanceof Error ? e.message : e}`,
            );
        }
    }

    private broadcastShot(
        matchId: string,
        seat: Seat,
        input: ShotInput,
        outcome: ShotOutcome,
        ...
        try {
            ...
            this.logger.debug(
                `shot broadcast skipped: ${e instanceof Error ? e.message : e}`,
            );
        }
    }
}
