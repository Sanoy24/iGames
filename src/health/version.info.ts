...
function resolvePackageVersion(): string {
  try {
    ...
    return require(join(process.cwd(), 'package.json')).version ?? 'unknown';
  } catch {
    return 'unknown';
  }
}
...
export const VERSION_INFO = Object.freeze({
  ...
  gitCommit: resolveGit(process.env.GIT_COMMIT ?? process.env.SOURCE_COMMIT, 'rev-parse --short HEAD'),
  gitBranch: resolveGit(process.env.GIT_BRANCH ?? process.env.SOURCE_BRANCH, 'rev-parse --abbrev-ref HEAD'),
