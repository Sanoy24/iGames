...
export type BotStrategyProfile =
    | 'conservative'
    | 'normal'
    | 'aggressive'
    | 'mirror-human';
...
export class BotsService {
    ...
    async listBots(): Promise<BotResponse[]> {
        ...
        return Promise.all(
            bots.map(async (b) => {
                ...
                try {
                    const ws = await this.walletService.getDefaultWalletSummary(
                        b.id,
                    );
                    walletBalanceMinor = ws.availableMinor;
                } catch {
                    /* bot may not have a wallet yet */
                }
    ...
    async deleteBot(botId: string): Promise<void> {
        ...
        const policy = this.normalizeBotPolicy(
            bot.productMetadata!.botPolicy as BotPolicyInput,
        );
        bot.productMetadata = {
            ...bot.productMetadata!,
            botPolicy: { ...policy, active: false },
        };
    ...
    async createBotName(dto: CreateBotNameDto): Promise<BotNameResponse> {
        ...
        const existing = await this.botNameRepository.findOneBy({
            displayName,
        });
        if (existing) {
    ...
    async importBotNames(dto: ImportBotNamesDto): Promise<BotNameResponse[]> {
        const normalized = [
            ...new Set(
                dto.names
                    .map((name) => this.normalizeBotName(name))
                    .filter(Boolean),
            ),
        ];
        ...
        const toCreate = normalized
            ...
            .map((displayName) =>
                this.botNameRepository.create({ displayName, active: true }),
            );

        if (toCreate.length === 0) return [];
        return (await this.botNameRepository.save(toCreate)).map((name) =>
            this.toBotNameResponse(name),
        );
    }

    async updateBotName(
        id: string,
        dto: UpdateBotNameDto,
    ): Promise<BotNameResponse> {
        ...
        if (dto.displayName !== undefined) {
            ...
            if (displayName !== name.displayName) {
                const conflict = await this.botNameRepository.findOneBy({
                    displayName,
                });
    ...
    async updatePolicy(
        ...
        const current = this.normalizeBotPolicy(
            bot.productMetadata!.botPolicy as BotPolicyInput,
        );
        const nextKeno = {
            ...current.games!.keno!,
            ...(dto.kenoActive !== undefined && { active: dto.kenoActive }),
            ...(dto.kenoStrategy !== undefined && {
                strategy: dto.kenoStrategy,
            }),
            ...(dto.kenoParticipationRatePct !== undefined && {
                participationRatePct: dto.kenoParticipationRatePct,
            }),
            ...(dto.kenoActionDelayMinMs !== undefined && {
                actionDelayMinMs: dto.kenoActionDelayMinMs,
            }),
            ...(dto.kenoActionDelayMaxMs !== undefined && {
                actionDelayMaxMs: dto.kenoActionDelayMaxMs,
            }),
            ...(dto.kenoHesitationChancePct !== undefined && {
                hesitationChancePct: dto.kenoHesitationChancePct,
            }),
            ...(dto.kenoVariancePct !== undefined && {
                variancePct: dto.kenoVariancePct,
            }),
            ...(dto.kenoMinBalanceMinor !== undefined && {
                minBalanceMinor: dto.kenoMinBalanceMinor,
            }),
            ...(dto.kenoMaxStakeMinorPerDay !== undefined && {
                maxStakeMinorPerDay: dto.kenoMaxStakeMinorPerDay,
            }),
            ...(dto.ticketsPerRound !== undefined && {
                ticketsPerRound: dto.ticketsPerRound,
            }),
        ...
        const updated: BotPolicy = {
            ...
            games: {
                ...
                bingo: {
                    ...current.games!.bingo!,
                    ...(dto.bingoActive !== undefined && {
                        active: dto.bingoActive,
                    }),
                    ...(dto.bingoStrategy !== undefined && {
                        strategy: dto.bingoStrategy,
                    }),
                    ...(dto.bingoParticipationRatePct !== undefined && {
                        participationRatePct: dto.bingoParticipationRatePct,
                    }),
                    ...(dto.bingoActionDelayMinMs !== undefined && {
                        actionDelayMinMs: dto.bingoActionDelayMinMs,
                    }),
                    ...(dto.bingoActionDelayMaxMs !== undefined && {
                        actionDelayMaxMs: dto.bingoActionDelayMaxMs,
                    }),
                    ...(dto.bingoHesitationChancePct !== undefined && {
                        hesitationChancePct: dto.bingoHesitationChancePct,
                    }),
                    ...(dto.bingoVariancePct !== undefined && {
                        variancePct: dto.bingoVariancePct,
                    }),
                    ...(dto.bingoMinBalanceMinor !== undefined && {
                        minBalanceMinor: dto.bingoMinBalanceMinor,
                    }),
                    ...(dto.bingoMaxStakeMinorPerDay !== undefined && {
                        maxStakeMinorPerDay: dto.bingoMaxStakeMinorPerDay,
                    }),
                    ...(dto.bingoMaxCartelasPerRoom !== undefined && {
                        maxCartelasPerRoom: dto.bingoMaxCartelasPerRoom,
                    }),
                },
                crash: {
                    ...current.games!.crash!,
                    ...(dto.crashActive !== undefined && {
                        active: dto.crashActive,
                    }),
                    ...(dto.crashStrategy !== undefined && {
                        strategy: dto.crashStrategy,
                    }),
                    ...(dto.crashParticipationRatePct !== undefined && {
                        participationRatePct: dto.crashParticipationRatePct,
                    }),
                    ...(dto.crashActionDelayMinMs !== undefined && {
                        actionDelayMinMs: dto.crashActionDelayMinMs,
                    }),
                    ...(dto.crashActionDelayMaxMs !== undefined && {
                        actionDelayMaxMs: dto.crashActionDelayMaxMs,
                    }),
                    ...(dto.crashHesitationChancePct !== undefined && {
                        hesitationChancePct: dto.crashHesitationChancePct,
                    }),
                    ...(dto.crashVariancePct !== undefined && {
                        variancePct: dto.crashVariancePct,
                    }),
                    ...(dto.crashMinBalanceMinor !== undefined && {
                        minBalanceMinor: dto.crashMinBalanceMinor,
                    }),
                    ...(dto.crashMaxStakeMinorPerDay !== undefined && {
                        maxStakeMinorPerDay: dto.crashMaxStakeMinorPerDay,
                    }),
                    ...(dto.crashMinCashoutX100 !== undefined && {
                        minCashoutX100: dto.crashMinCashoutX100,
                    }),
                    ...(dto.crashMaxCashoutX100 !== undefined && {
                        maxCashoutX100: dto.crashMaxCashoutX100,
                    }),
                },
            },
        };
    ...
    async buyTicketsForDraw(drawId: string): Promise<void> {
        ...
        if (interval > 0) {
            ...
            if (lastForcedTicket) {
                const lastDraw = await kenoDrawRepo.findOneBy({
                    id: lastForcedTicket.drawId,
                });
                if (lastDraw) {
            ...
            if (drawsSinceLast >= interval - 1) {
                ...
                this.logger.log(
                    `Global interval reached (${interval}). Bot ${forcedBotId} chosen for forced win.`,
                );
            }
        }
    ...
    async topUpBotsForOpenRoom(roomId: string): Promise<boolean> {
        ...
        const referencePolicy = this.normalizeBotPolicy(
            bots[randomInt(0, bots.length)].productMetadata!
                .botPolicy as BotPolicyInput,
        );
        await this.pause(
            this.resolveActionDelay(
                referencePolicy.games!.bingo!,
                'bingo',
                'reconcile',
            ),
        );
        ...
        const changed =
            await this.bingoService.reconcileBotCartelasInRoom(roomId);
        if (changed) {
            await this.logBotAction({
                botId: null,
                game: 'bingo',
     ...
     */
    async handleBingoBotWinInterval(
        roomId: string,
        policyOrInterval:
            | number
            | {
        ...
        const policy =
            typeof policyOrInterval === 'number'
                ? {
                ...
                : {
                      ...
                      everyNRounds: Math.max(
                          0,
                          policyOrInterval.everyNRounds ?? 0,
                      ),
                      chancePct: Math.min(
                          100,
                          Math.max(0, policyOrInterval.chancePct ?? 0),
                      ),
                  };
        ...
        if (
            ...
            (policy.everyNRounds <= 0 ||
                completedCount % policy.everyNRounds !== 0)
        )
            return;
        if (
            policy.mode === 'random' &&
            (policy.chancePct <= 0 || randomInt(0, 100) >= policy.chancePct)
        )
            return;
        ...
        try {
            const luckyPolicy = this.normalizeBotPolicy(
                luckyBot.productMetadata!.botPolicy as BotPolicyInput,
            );
            await this.pause(
                this.resolveActionDelay(
                    luckyPolicy.games!.bingo!,
                    'bingo',
                    'bonus',
                ),
            );
            ...
            this.logger.error(
                `Failed to credit bot win for room ${roomId}`,
    ...
    async placeBetsForCrashRound(roundId: string): Promise<void> {
        ...
        for (const bot of bots) {
            ...
            if (!this.rollParticipation(crashPolicy.participationRatePct))
                continue;
            if (
                !(await this.canBotEnterGame(
                    bot,
                    'crash',
                    crashPolicy,
                    cfg.botBetMinor,
            ...
            const autoCashoutX100 = this.humanizeCrashCashout(
                minCashout,
                maxCashout,
                crashPolicy,
            );
            ...
            try {
                const bet = await this.crashService.placeBet(
                    ...
                    roundId,
                    {
                        stakeMinor: cfg.botBetMinor,
                        autoCashoutAt: autoCashoutX100 / 100,
                    },
                    idempotencyKey,
                );
                await this.logBotAction({
    ...
    private async getActiveBots(
        ...
        const bots = await this.userRepository
            ...
            .andWhere(
                "JSON_EXTRACT(user.productMetadata, '$.botPolicy.active') = true",
            )
            .getMany();
        if (!game) return bots;
        if (game === 'bingo') {
            return bots.filter(
                (bot) =>
                    (bot.productMetadata!.botPolicy as BotPolicyInput).games
                        ?.bingo?.active === true &&
                    this.isGameEnabled(
                        this.normalizeBotPolicy(
                            bot.productMetadata!.botPolicy as BotPolicyInput,
                        ),
                        game,
                    ),
    ...
    private async buyTicketsForSingleBot(
        bot: User,
        drawId: string,
        isForcedWin: boolean,
    ): Promise<void> {
        ...
        if (
            !(await this.canBotEnterGame(
                ...
                'keno',
                kenoPolicy,
                config.ticketPriceMinor * kenoPolicy.ticketsPerRound,
            ))
        )
            return;
        ...
        const spotCount = allowedSpots.includes(kenoPolicy.spotCount)
            ...
            : allowedSpots[0];
        const ticketsToBuy = this.humanizeActionCount(
        ...
        await this.pause(
            this.resolveActionDelay(kenoPolicy, 'keno', 'purchase'),
        );

        for (let i = 0; i < ticketsToBuy; i++) {
            if (i > 0) {
                await this.pause(
                    this.resolveActionDelay(kenoPolicy, 'keno', 'purchase', i),
                );
            }
            const selectedNumbers = this.pickRandomNumbers(
                numberMin,
                numberMax,
                spotCount,
            );
    ...
    private humanizeActionCount(
        ...
        const spread = Math.max(
            0,
            Math.round((safeBase * Math.max(variancePct, 0)) / 100),
        );
    ...
    private resolveActionDelay(
        ...
        const maxDelay = this.clampInt(
            Math.max(policy.actionDelayMaxMs, minDelay),
            minDelay,
            15_000,
        );
        const baseDelay =
            minDelay === maxDelay
                ? minDelay
                : randomInt(minDelay, maxDelay + 1);
        const varianceWindow = Math.max(
            0,
            Math.round(baseDelay * (Math.max(policy.variancePct, 0) / 100)),
        );
        const variance =
            varianceWindow > 0
                ? randomInt(0, varianceWindow * 2 + 1) - varianceWindow
                : 0;
        const sequenceSpread =
            game === 'bingo' ? 140 : game === 'crash' ? 90 : 160;
        const hesitationChance = this.clampInt(
            policy.hesitationChancePct,
            0,
            100,
        );
        const shouldHesitate =
            hesitationChance > 0 && randomInt(0, 100) < hesitationChance;
        const hesitation = shouldHesitate
            ? randomInt(
                  Math.max(40, Math.floor(minDelay / 2)),
                  Math.max(maxDelay, minDelay + 1) + 700,
              )
            : 0;
        const kindBias =
            kind === 'bonus'
    ...
    private humanizeCrashCashout(
        ...
        const spread = Math.max(
            1,
            Math.round((range * Math.max(policy.variancePct, 5)) / 100),
        );
    ...
    private async canBotEnterGame(
        ...
        if (policy.minBalanceMinor > 0) {
            try {
                const wallet = await this.walletService.getDefaultWalletSummary(
                    bot.id,
                );
                if (wallet.availableMinor < policy.minBalanceMinor) {
                    await this.logBotAction({
                        ...
                        metadata: {
                            ...
                            minBalanceMinor: policy.minBalanceMinor,
                        },
                    });
                    return false;
                }
        ...
        if (policy.maxStakeMinorPerDay > 0) {
            ...
            const raw = await this.botActionLogRepository
                ...
                .andWhere('log.action IN (:...actions)', {
                    actions: ['keno_ticket_purchase', 'crash_bet_placed'],
                })
            ...
            if (spentToday + plannedStakeMinor > policy.maxStakeMinorPerDay) {
                await this.logBotAction({
                    ...
                    metadata: {
                        spentToday,
                        maxStakeMinorPerDay: policy.maxStakeMinorPerDay,
                    },
                });
                return false;
            }
    ...
    private async logBotAction(
        ...
        try {
            const repo = manager
                ? manager.getRepository(BotActionLog)
                : this.botActionLogRepository;
            await repo.save(
                repo.create({
                    botId: input.botId,
                    game: input.game,
            ...
            this.logger.warn(
                `Failed to write bot action log: ${input.action}`,
                error instanceof Error ? error.stack : error,
            );
        }
    }

    private pickRandomNumbers(
        min: number,
        max: number,
        count: number,
    ): number[] {
    ...
    private async findBot(botId: string): Promise<User> {
        ...
        const bot = await this.userRepository
            .createQueryBuilder('user')
    ...
    private toBotResponse(user: User): BotResponse {
        return {
            ...
            botPolicy: this.normalizeBotPolicy(
                user.productMetadata!.botPolicy as BotPolicyInput,
            ),
        };
    }
    ...
    private normalizeBotPolicy(policy: BotPolicyInput = {}): BotPolicy {
        const ticketsPerRound =
            policy.ticketsPerRound ?? policy.games?.keno?.ticketsPerRound ?? 1;
        const spotCount =
            policy.spotCount ?? policy.games?.keno?.spotCount ?? 3;
        const drawParticipationCount =
            policy.drawParticipationCount ??
            policy.games?.keno?.drawParticipationCount ??
            0;
        ...
        const base = (
            ...
            participationRatePct: this.clampInt(
                gamePolicy?.participationRatePct ??
                    defaults.participationRatePct,
                0,
                100,
            ),
            actionDelayMinMs: this.clampInt(
                gamePolicy?.actionDelayMinMs ?? defaults.actionDelayMinMs,
                0,
                10_000,
            ),
            actionDelayMaxMs: this.clampInt(
                Math.max(
                    gamePolicy?.actionDelayMaxMs ?? defaults.actionDelayMaxMs,
                    gamePolicy?.actionDelayMinMs ?? defaults.actionDelayMinMs,
                ),
                0,
                15_000,
            ),
            hesitationChancePct: this.clampInt(
                gamePolicy?.hesitationChancePct ?? defaults.hesitationChancePct,
                0,
                100,
            ),
            variancePct: this.clampInt(
                gamePolicy?.variancePct ?? defaults.variancePct,
                0,
                100,
            ),
            minBalanceMinor: Math.max(0, gamePolicy?.minBalanceMinor ?? 0),
            maxStakeMinorPerDay: Math.max(
                0,
                gamePolicy?.maxStakeMinorPerDay ?? 0,
            ),
            maxLossMinorPerDay: Math.max(
                0,
                gamePolicy?.maxLossMinorPerDay ?? 0,
            ),
            maxWinMinorPerDay: Math.max(0, gamePolicy?.maxWinMinorPerDay ?? 0),
        });
        const kenoBase = base(policy.games?.keno, {
            strategy: 'normal',
        ...
        const bingoBase = base(policy.games?.bingo, {
        ...
        const crashBase = base(policy.games?.crash, {
            strategy: 'normal',
        ...
        const minCashoutX100 = Math.max(
            101,
            policy.games?.crash?.minCashoutX100 ?? 120,
        );
        const maxCashoutX100 = Math.max(
            minCashoutX100,
            policy.games?.crash?.maxCashoutX100 ?? 250,
        );
        return {
            ...policy,
            ticketsPerRound,
            ...
            games: {
                ...
                bingo: {
                    ...bingoBase,
                    maxCartelasPerRoom: this.clampInt(
                        policy.games?.bingo?.maxCartelasPerRoom ?? 24,
                        1,
                        24,
                    ),
                },
    ...
    private isGameEnabled(
        policy: BotPolicy,
        game: 'keno' | 'bingo' | 'crash',
    ): boolean {
        return (
            policy.active !== false && policy.games?.[game]?.active !== false
        );
    }
    ...
    private toBotActionResponse(log: BotActionLog): BotActionLogResponse {
        return {
            ...
            amountMinor:
                log.amountMinor == null ? null : Number(log.amountMinor),
