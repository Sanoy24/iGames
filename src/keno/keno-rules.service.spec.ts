...
describe('KenoRulesService', () => {
    ...
    describe('validateSelectedNumbers', () => {
        it('accepts valid unique selected numbers', () => {
            expect(() =>
                service.validateSelectedNumbers([1, 2, 3], config),
            ).not.toThrow();
        });

        it('accepts boundary values (1 and 80)', () => {
            expect(() =>
                service.validateSelectedNumbers([1, 80, 40], config),
            ).not.toThrow();
        });

        it('rejects duplicate selected numbers', () => {
            expect(() =>
                service.validateSelectedNumbers([1, 1, 2], config),
            ).toThrow(BadRequestException);
        });

        it('rejects out-of-range selected numbers (above max)', () => {
            expect(() =>
                service.validateSelectedNumbers([1, 2, 81], config),
            ).toThrow(BadRequestException);
        });

        it('rejects out-of-range selected numbers (below min = 0)', () => {
            expect(() =>
                service.validateSelectedNumbers([0, 1, 2], config),
            ).toThrow(BadRequestException);
        });
    ...
    describe('countMatches', () => {
        ...
        it('returns full match when all selected numbers are drawn', () => {
            expect(
                service.countMatches([10, 20, 30], [10, 20, 30, 40, 50]),
            ).toBe(3);
        });

        it('returns correct match with 5-spot selection', () => {
            expect(
                service.countMatches([3, 15, 27, 44, 68], [3, 15, 27, 44, 99]),
            ).toBe(4);
        });
    });
