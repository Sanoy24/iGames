    ...
    implements OnApplicationBootstrap, OnApplicationShutdown
{
    ...
    private async runTick(): Promise<void> {
        ...
        if (!active) {
            if (
                this.lastCrashedAt &&
                now - this.lastCrashedAt < this.betweenRoundDelayMs
            )
                return;
            ...
            void this.botsService
                .placeBetsForCrashRound(round.id)
                .catch(() => {});
            return;
        }
        ...
        if (active.status === 'waiting') {
            ...
            try {
                ...
                if (!(err instanceof ConflictException)) {
                    this.logger.error(
                        `Failed to start crash round ${active.id}`,
                        err instanceof Error ? err.stack : err,
                    );
                }
            }
        ...
        if (active.status === 'running' && active.startedAt) {
            ...
            void this.crashService
                .processAutoCashouts(active.id, multiplierX100)
                .catch(() => {});
            ...
            if (
                active.crashPointX100 !== null &&
                multiplierX100 >= active.crashPointX100
            ) {
    ...
    private async handleCrash(
        ...
        try {
            ...
            const round = await this.crashService.settleRound(
                roundId,
                elapsedMs,
            );
