...
import {
    ...
    PlayerDrillDown,
    ReferralCard,
} from '../components/AgentAreaViews';
...
export function Agent() {
    ...
    const [dashboard, setDashboard] = useState<AgentDashboardSummary | null>(
        null,
    );
    ...
    const [proofProvider, setProofProvider] = useState<
        Record<string, 'telebirr' | 'mpesa'>
    >({});
    const [receiptFiles, setReceiptFiles] = useState<Record<string, File>>({});
    const [transferCompletedAtInputs, setTransferCompletedAtInputs] = useState<
    ...
    const [view, setView] = useState<
        'withdrawals' | 'area' | 'settlements' | 'support'
    >('withdrawals');
    const [selectedAreaPlayer, setSelectedAreaPlayer] = useState<string | null>(
        null,
    );
    ...
    const handleTransferToUser = async (e: React.FormEvent) => {
        ...
        try {
            const result = await agentApi.transferToUser(
                transferPhone.trim(),
                amount,
            );
            ...
            addToast(
                'success',
                `Transferred ${formatCreditsFull(amount)} to ${transferPhone}`,
            );
            setTransferAmount('');
            setTransferPhone('');
        } catch (err) {
    ...
    const handleRequestSettlement = async () => {
        ...
        try {
            ...
            addToast(
                'success',
                t('agent.settlementRequested', {
                    defaultValue:
                        'Settlement requested  an admin will review it shortly.',
                }),
            );
            agentApi
                .getDashboard()
    ...
    const handleClaim = async (id: string) => {
        ...
        try {
            ...
            addToast(
                'success',
                'Withdrawal claimed  make the Telebirr transfer now.',
            );
        } catch (err) {
            addToast('error', getErrorMessage(err));
        } finally {
    ...
    const handleComplete = async (w: Withdrawal) => {
        ...
        const transferCompletedAtLocal =
            transferCompletedAtInputs[w.id] || nowLocalInputValue();
        setBusyFor(w.id, true);
        try {
            const { fileUrl } =
                await agentApi.uploadWithdrawalReceipt(receiptFile);
            await agentApi.completeWithdrawal(
                w.id,
                provider,
                proof,
                fileUrl,
                new Date(transferCompletedAtLocal).toISOString(),
            );
            await load();
            addToast(
                'success',
                'Payout proof submitted  awaiting admin verification.',
            );
        } catch (err) {
            addToast('error', getErrorMessage(err));
        } finally {
    ...
    const feeOf = (gross: number): number => {
        const match = config?.withdrawalFeeRanges.find(
            (r) =>
                gross >= r.minAmountMinor &&
                (r.maxAmountMinor === null || gross <= r.maxAmountMinor),
        );
    ...
    return (
        ...
        >
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
                <button
                    className={`btn btn-sm ${view === 'withdrawals' ? 'btn-primary' : 'btn-ghost'}`}
                    style={{ flex: 1 }}
                ...
                <button
                    className={`btn btn-sm ${view === 'area' ? 'btn-primary' : 'btn-ghost'}`}
                    style={{ flex: 1 }}
                    onClick={() => setView('area')}
                >
                    <MapPin size={14} />{' '}
                    {t('agent.areaTab', { defaultValue: 'My Area' })}
                </button>
                <button
                    className={`btn btn-sm ${view === 'settlements' ? 'btn-primary' : 'btn-ghost'}`}
                    style={{ flex: 1 }}
                    onClick={() => setView('settlements')}
                >
                    <Clock size={14} />{' '}
                    {t('agent.settlementsTab', { defaultValue: 'Settlements' })}
                </button>
                <button
                    className={`btn btn-sm ${view === 'support' ? 'btn-primary' : 'btn-ghost'}`}
                    style={{ flex: 1 }}
            ...
            {view === 'area' &&
                (selectedAreaPlayer ? (
                    <PlayerDrillDown
                        userId={selectedAreaPlayer}
                        onBack={() => setSelectedAreaPlayer(null)}
                    />
                ) : (
                    <>
                        <ReferralCard />
                        <AreaPlayerList
                            onSelectPlayer={setSelectedAreaPlayer}
                        />
                    </>
                ))}
                ...
                <>
                    ...
                    >
                        {[
                            {
                                key: 'balance',
                                label: t('agent.balance'),
                                value: formatCredits(
                                    agentWallet?.availableMinor ?? 0,
                                ),
                                accent: true,
                            },
                            {
                                key: 'feeComm',
                                label: t('agent.feeComm'),
                                value: `${config?.withdrawalFeeRanges.length ?? 0} tiers`,
                            },
                            {
                                key: 'pool',
                                label: t('agent.pool'),
                    ...
                    {dashboard && (
                        ...
                        >
                            ...
                            >
                                {[
                                    {
                                        label: t('agent.referredPlayers', {
                                            defaultValue: 'Referred',
                                        }),
                                        value: String(
                                            dashboard.totalReferredPlayers,
                                        ),
                                    },
                                    {
                                        label: t('agent.activePlayers', {
                                            defaultValue: 'Active (30d)',
                                        }),
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        {t('agent.gameCommission', {
                                            defaultValue: 'Game Commission',
                                        })}
                                    </div>
                                    <div
                                        style={{
                                    ...
                                    >
                                        {formatCredits(
                                            dashboard.gameCommission.totalMinor,
                                        )}
                                    </div>
                                    <div
                                        style={{
                                    ...
                                    >
                                        {t('agent.commissionCount', {
                                            count: dashboard.gameCommission
                                                .count,
                                            defaultValue: `${dashboard.gameCommission.count} commissions`,
                                        })}
                                    </div>
                                </div>
                                <div
                                ...
                                >
                                    ...
                                    >
                                        {t('agent.withdrawalFees', {
                                            defaultValue: 'Withdrawal Fees',
                                        })}
                                    </div>
                                    <div
                                        style={{
                                    ...
                                    >
                                        {formatCredits(
                                            dashboard.withdrawalFeesEarnedMinor,
                                        )}
                                    </div>
                                </div>
                            </div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        {t('agent.totalSettled', {
                                            defaultValue: 'Total Settled',
                                        })}
                                    </div>
                                    <div
                                        style={{
                            ...
                            <div style={{ marginBottom: 12 }}>
                                ...
                                >
                                    {requestingSettlement
                                        ? t('agent.requestingSettlement', {
                                              defaultValue: 'Requesting…',
                                          })
                                        : t('agent.requestSettlement', {
                                              defaultValue:
                                                  'Request Settlement',
                                          })}
                                </button>
                                {!dashboard.canRequestSettlement && (
                                    ...
                                    >
                                        {dashboard.settlementBlockReason ===
                                            'pending_exists' &&
                                            t('agent.settlementPendingReview', {
                                                defaultValue:
                                                    'You already have a settlement request awaiting review.',
                                            })}
                                        {dashboard.settlementBlockReason ===
                                            'cooldown' &&
                                            dashboard.settlementCooldownEndsAt &&
                                        ...
                                        {dashboard.settlementBlockReason ===
                                            'nothing_to_settle' &&
                                            t(
                                                'agent.settlementNothingToSettle',
                                                {
                                                    defaultValue:
                                                        'No new earnings to settle yet.',
                                                },
                                            )}
                                    </div>
                                )}
                            ...
                            >
                                <div style={{ textAlign: 'center' }}>
                                    ...
                                    >
                                        {dashboard.pendingWithdrawalRequests}
                                    </div>
                                </div>
                                <div style={{ textAlign: 'center' }}>
                                    <div
                                    ...
                                    >
                                        {dashboard.completedWithdrawalRequests}
                                    </div>
                                </div>
                            </div>
                        </div>
          ...
          a different population than this room's own traffic. */}
                    {perf &&
                        (perf.tickets > 0 || perf.customersBrought > 0) && (
                            <div
                                style={{
                                    background: 'var(--card-bg)',
                                    border: '1px solid var(--border)',
                                    ...
                                    {[
                                        {
                                            ...
                                            value: String(
                                                perf.customersBrought,
                                            ),
                                        },
                                        {
                                            label: t('agent.players', {
                                                defaultValue: 'Players',
                                            }),
                                            value: String(perf.players),
                                        },
                                        {
                                            label: t('agent.staked', {
                                                defaultValue: 'Staked',
                                            }),
                                            value: formatCredits(
                                                perf.stakedMinor,
                    ...
                    {perf &&
                        (perf.depositCount > 0 ||
                            perf.depositCommissionEarnedMinor > 0) && (
                            <div
                                style={{
                                    background: 'var(--card-bg)',
                                    border: '1px solid var(--border)',
                            ...
                            >
                                ...
                                >
                                    {t('agent.myDeposits', {
                                        defaultValue: 'My Deposit Activity',
                                    })}
                                </div>
                                <div
                                    style={{
                                ...
                                >
                                    {[
                                        {
                                            label: t('agent.depositCount', {
                                                defaultValue: 'Deposits',
                                            }),
                                            value: String(perf.depositCount),
                                        },
                                        {
                                            label: t('agent.depositVolume', {
                                                defaultValue: 'Volume',
                                            }),
                                            value: formatCredits(
                                                perf.depositVolumeMinor,
                                            ),
                                        },
                                        {
                                            label: t(
                                            ...
                                            value: formatCredits(
                                                perf.depositCommissionEarnedMinor,
                                            ),
                                            accent: true,
                                        },
                                    ].map((s) => (
                    ...
                    >
                        ...
                        >
                            Transfer ETB to Player
                        </div>
                        <form
                            onSubmit={handleTransferToUser}
                            style={{
                                display: 'flex',
                    ...
                    <div style={{ marginBottom: 12 }}>
                        ...
                        {loading && mine.length === 0 ? (
                            ...
                            >
                                No active requests claim one from the pool
                                below.
                            </div>
                        ) : (
                            <div
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <span
                                                    className={`badge ${STATUS_BADGE[w.status] ?? 'badge-gold'}`}
                                                >
                                                    {w.status}
                                                </span>
                                            </div>
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <Step
                                                        n={2}
                                                        label={`Send the exact "You transfer" amount to ${w.destinationAccount}`}
                                                    />
                                                    <Step
                                                        n={3}
                                                        label='Paste your payment proof below to verify & complete'
                                                    />
                                                </div>
                                                ...
                                                {w.status !== 'claimed' ? (
                                                    ...
                                                    >
                                                        ...
                                                        waiting on an admin to
                                                        verify the FT number and
                                                        receipt.
                                                    </div>
                                                ) : (
                                                    <>
