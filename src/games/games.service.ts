...
import {
    ...
    GameSetting,
    GameState,
} from './entities/game-setting.entity';

/** Canonical list of games and their default display order. */
const GAME_DEFAULTS: Array<{
    code: GameCode;
    name: string;
    displayOrder: number;
}> = [
    ...
    { code: 'werk', name: 'Werk Flega', displayOrder: 4 },
];

const GAME_NAME: Record<GameCode, string> = {
    keno: 'Keno',
    bingo: 'Bingo',
    crash: 'Crash',
    pool: 'Pool',
...
export class GamesService implements OnApplicationBootstrap {
        ...
        try {
            ...
            if (!(await qr.hasTable(meta.tableName))) {
                await qr.createTable(
                    Table.create(meta, this.dataSource.driver),
                    true,
                );
            ...
            this.logger.warn(
                `ensureTable failed: ${err instanceof Error ? err.message : err}`,
            );
        } finally {
            await qr.release();
        }
    ...
    private async ensureSeeded(): Promise<void> {
        ...
        try {
            ...
            this.logger.warn(
                `Game settings seed skipped: ${err instanceof Error ? err.message : err}`,
            );
        }
    }
        ...
        const msg =
            row.state === 'maintenance'
                ? row.maintenanceMessage?.trim() ||
                  `${GAME_NAME[code]} is under maintenance. Please try again later.`
    ...
    async updateSetting(
        ...
        if (dto.maintenanceMessage !== undefined)
            row.maintenanceMessage = dto.maintenanceMessage?.trim() || null;
