...
export class AgentMatchController {
  ...
  @Patch('me')
  setMine(@CurrentUser() user: AuthenticatedUser, @Body() dto: SetAssignedAgentDto) {
  ...
  async attempt(@CurrentUser() user: AuthenticatedUser, @Body() dto: MatchAgentFromCoordsDto) {
    const match = await this.usersService.matchAgentFromCoords(dto.latitude, dto.longitude);
    if (!match) return null;
    return this.usersService.setAssignedAgent(user.id, { agentId: match.id }, 'gps_match');
  }
}
