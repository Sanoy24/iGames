...
function makeService(input: {
  message?: Partial<{ id: string; requestType: string | null; requestStatus: string | null; requestedAmountMinor: number | null; ticketId: string; relatedType: string | null; relatedId: string | null }>;
  ...
  const adminService = {
    creditFromMasterWallet: input.creditFromMasterWallet ?? jest.fn().mockResolvedValue({
      wallet: {},
  ...
  const notifications = { safeCreate: jest.fn().mockResolvedValue(undefined) };
...
describe('SupportService — approveRefundRequest is Master-Wallet-backed', () => {
  it('credits the ticket owner from the Master Wallet, inside a transaction', async () => {
    ...
    await service.approveRefundRequest('agent-1', 'msg-1', { amountMinor: 3000 } as any);
  ...
  it('rejects an amount exceeding the requested amount before ever touching the Master Wallet', async () => {
    ...
    await expect(
      service.approveRefundRequest('agent-1', 'msg-1', { amountMinor: 999999 } as any),
