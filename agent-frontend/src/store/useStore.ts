...
export const useStore = create<AppState>((set) => ({
    ...
    setAuthLoading: () =>
        set({ authStatus: 'loading', isAuthLoading: true, authError: null }),
    ...
    addToast: (type, message) => {
        ...
        set((s) =>
            s.toasts.some((t) => t.message === message)
                ? s
    ...
    soundVolume:
        typeof window !== 'undefined'
            ? Number(localStorage.getItem('soundVolume') ?? '0.5')
            : 0.5,
    soundMuted:
        typeof window !== 'undefined'
            ? localStorage.getItem('soundMuted') === '1'
            : false,
    setSoundVolume: (vol) => {
...
export const formatCredits = (minor: number) => {
    ...
    if (minor >= 1_000)
        return `${(minor / 1_000).toFixed(minor % 1000 === 0 ? 0 : 1)}K`;
