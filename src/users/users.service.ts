...
export class UsersService {
    ...
    async findOrCreateTelegramUser(
        ...
        const authRepo = manager
            ? manager.getRepository(AuthIdentity)
            : this.authIdentityRepository;
        const userRepo = manager
            ? manager.getRepository(User)
            : this.userRepository;

        const existingIdentity = await authRepo.findOneBy({
            provider,
            providerUserId,
        });
        ...
        if (existingIdentity) {
            ...
            if (!this.isPlayerUser(existingUser)) {
                ...
                existingIdentity.providerUsername =
                    input.username?.toLowerCase();
                existingIdentity.profileSnapshot =
                    this.toTelegramSnapshot(input);
    ...
    private isPlayerUser(user: User): boolean {
        return (
            Array.isArray(user.roles) && user.roles.includes('player' as any)
        );
    }
    ...
    async listUsers(
        ...
        if (role) {
            queryBuilder.andWhere('JSON_CONTAINS(user.roles, :role)', {
                role: `"${role}"`,
            });
        }
        ...
        if (filters.isBot === true) {
            ...
            queryBuilder.andWhere(
                "JSON_EXTRACT(user.productMetadata, '$.botPolicy') IS NULL",
            );
        }
        ...
        if (filters.minBalanceMinor !== undefined) {
            queryBuilder.andWhere('wallet.availableMinor >= :minBalance', {
                minBalance: filters.minBalanceMinor,
            });
        }
        if (filters.maxBalanceMinor !== undefined) {
            queryBuilder.andWhere('wallet.availableMinor <= :maxBalance', {
                maxBalance: filters.maxBalanceMinor,
            });
        }
        ...
        if (filters.online === true) {
            ...
            queryBuilder.andWhere('user.id IN (:...onlineIds)', {
                onlineIds: ids,
            });
        } else if (
            filters.online === false &&
            filters.onlineUserIds &&
            filters.onlineUserIds.length > 0
        ) {
            queryBuilder.andWhere('user.id NOT IN (:...onlineIds)', {
                onlineIds: filters.onlineUserIds,
            });
        }
    ...
    async createAgentUser(input: {
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            const existingForPhone = await this.findLivePasswordIdentities(
                normalizedPhone,
                manager,
            );
            if (
                existingForPhone.some(
                    ({ user }) =>
                        ...
                        user.roles.includes('agent' as any),
                )
            ) {
                throw new ConflictException(
                    'An agent with that phone number already exists',
                );
            }
            const providerUserId = this.buildScopedProviderUserId(
                normalizedPhone,
                'agent',
                existingForPhone,
            );
    ...
    async ensureAdminAccount(input: {
        ...
        if (!normalizedPhone)
            throw new BadRequestException(
                `Invalid admin bootstrap phone number: ${input.phoneNumber}`,
            );
    ...
    async updateAgentUser(
        ...
        if (update.workDaysOfWeek !== undefined)
            user.workDaysOfWeek = update.workDaysOfWeek;
        if (update.agentPermissions !== undefined)
    ...
    async listOnDutyAgentsForMatching(): Promise<
    ...
    async setAssignedAgent(
        ...
        if (wantsOther && dto.agentId) {
            throw new BadRequestException(
                'Send either agentId or other, not both',
            );
        }
        if (!wantsOther && !dto.agentId) {
            throw new BadRequestException(
                'Send an agentId, or other: true to skip attribution',
            );
        }
        ...
        if (!agent)
            throw new NotFoundException(
                'That agent is not currently available',
            );
     ...
     */
    private async allocateReferralCode(
        manager?: EntityManager,
    ): Promise<string> {
        const userRepo = manager
            ? manager.getRepository(User)
     ...
     */
    async countActiveReferredPlayers(
        agentId: string,
        sinceDate: Date,
    ): Promise<number> {
    ...
    async hasSharedLocation(agentId: string): Promise<boolean> {
        ...
        return (
            !!agent &&
            agent.sharedLatitude != null &&
            agent.sharedLongitude != null
        );
    }
    ...
    async findBackofficeUserByCredentials(
        ...
        const normalizedPhone =
            normalizeEthiopianPhone(rawTrimmed) ?? rawTrimmed;
        ...
        const candidateLists = await Promise.all(
            candidatePhones.map((phone) =>
                this.findLivePasswordIdentities(phone),
            ),
        );
        ...
        if (!hasBackofficeRole) {
            throw new BadRequestException(
                'Account does not have backoffice access',
            );
        }
     ...
     */
    async linkTelegramIdentityToUser(
        userId: string,
        input: TelegramIdentityInput,
        role: string,
    ): Promise<AuthIdentity> {
        const now = new Date();
        const existingForTelegramId = await this.findLiveTelegramIdentities(
            input.telegramUserId,
        );
        const providerUserId = this.buildTelegramProviderUserId(
            input.telegramUserId,
            role,
        );

        const own = existingForTelegramId.find(
            ({ user, identity }) =>
                user.id === userId &&
     ...
     */
    async findAgentPhoneByTelegramId(
        telegramUserId: string,
    ): Promise<{ phoneNumber: string; displayName: string } | null> {
        const identities =
            await this.findLiveTelegramIdentities(telegramUserId);
        const match = identities.find(
            ({ user }) =>
                Array.isArray(user.roles) &&
                user.roles.includes('agent' as any),
    ...
    async updateProfile(
        userId: string,
        update: { displayName?: string; phoneNumber?: string },
    ): Promise<User> {
        const user = await this.userRepository.findOneBy({ id: userId });
    ...
    async updatePhoneByTelegramId(
        ...
        if (!identity) return;
        await this.userRepository.update(identity.userId, {
