...
This repo is building an audit-conscious NestJS + MongoDB backend for Keno and 90-ball Bingo. Treat the backend as ledger-ready credits from day one: no real-money payments, KYC, AML, tax, or jurisdiction-specific compliance should be added unless explicitly requested. The first client may be a Telegram Bot and Telegram Mini App, but the backend must remain standalone and saleable as its own product.

## Stack And Shape

- Use NestJS, TypeScript, MongoDB, and Mongoose.
- Keep module boundaries clear: `auth`, `users`, `telegram`, `wallet`, `ledger`, `keno`, `bingo`, `rng`, `admin`, and `common`.
- Prefer DTOs, services, repositories/models, guards, interceptors, and pipes that match standard NestJS patterns.
...
- Never update a wallet balance without creating an immutable ledger entry in the same MongoDB transaction.
...
- Game rules, paytables, room settings, ticket prices, and prize values must be MongoDB-backed configuration, not hardcoded business values.
...
- Use MongoDB unique indexes/status guards to protect scheduled jobs from duplicate execution.
...
- Do not bypass ledger services by mutating wallet documents directly.
- Do not hardcode temporary game odds or prize values into services. Add seed data or config documents instead.
