    ...
    implements OnApplicationBootstrap, OnModuleDestroy
{
    ...
    constructor(
        @InjectRepository(WerkRound)
        private readonly roundRepo: Repository<WerkRound>,
        @InjectRepository(WerkParticipant)
        private readonly participantRepo: Repository<WerkParticipant>,
    ...
    onApplicationBootstrap(): void {
        ...
        this.gateway.registerWerkInputHandler((userId, input) =>
            this.handleInput(userId, input),
        );
        ...
        this.inputSub.on('message', (channel: string, message: string) => {
            ...
            try {
                const { userId, input } = JSON.parse(message) as {
                    userId: string;
                    input: WerkInput;
                };
        ...
        this.inputSub
            .subscribe(WERK_INPUT_CHANNEL)
            .catch((err) =>
                this.logger.error(
                    `Werk input subscribe failed: ${err instanceof Error ? err.message : err}`,
                ),
            );
    }
    ...
    async lifecycle(): Promise<void> {
        ...
        const playable = await this.gamesService
            .isPlayable('werk')
            .catch(() => false);
        ...
        if (r.status === 'lobby') {
            if (r.scheduledStartAt && now >= r.scheduledStartAt.getTime()) {
                ...
                this.live.humans.size === 0 &&
                this.gateway.werkPresenceCount() === 0
            ) {
    ...
    private stepHuman(live: LiveRound, h: HumanRuntime, dt: number): void {
        ...
        if (h.usePowerLatch) {
            if (h.boost <= 0 && h.pendingSpeed) {
                h.boost = SPEED_BOOST_SEC;
                h.pendingSpeed = false;
            } else if (h.magnet <= 0 && h.pendingMagnet) {
                h.magnet = MAGNET_SEC;
                h.pendingMagnet = false;
            } else if (h.shield <= 0 && h.pendingShield) {
                h.shield = SHIELD_SEC;
                h.pendingShield = false;
            }
            h.usePowerLatch = false;
        }

        const ax = h.input.moveX ?? 0,
            ay = h.input.moveY ?? 0;
    ...
    private async loadOrCreateActive(
        cfg: WerkConfig,
        playable: boolean,
    ): Promise<void> {
        const active = await this.roundRepo.findOne({
            where: { activeGuard: 1 },
        });
        if (active) {
        ...
        if (playable && cfg.enabled && this.gateway.werkPresenceCount() > 0)
            await this.ensureLobby(cfg);
    }
    ...
    private hydrate(round: WerkRound): LiveRound {
        ...
        const shared: SharedCoinPool = {
            ...
            take: (i, byId) => {
                if (coinOwner.has(i)) return false;
                coinOwner.set(i, byId);
                return true;
            },
        };
        const botSim = new BotSim(
            layout,
            roster as BotConfig[],
            {
                mode: round.mode,
    ...
    private async rehydrateParticipants(live: LiveRound): Promise<void> {
        const rows = await this.participantRepo.find({
            where: { roundId: live.round.id, status: 'joined' },
        });
        for (const p of rows)
            this.addHumanRuntime(live, p, await this.resolveName(p.userId));
    }

    private async resolveName(userId: string): Promise<string> {
        const u = await this.userRepo
            .findOne({ where: { id: userId }, select: ['displayName'] })
            .catch(() => null);
        return u?.displayName ?? 'Player';
    }

    private addHumanRuntime(
        live: LiveRound,
        p: WerkParticipant,
        displayName?: string,
    ): void {
        ...
        const ox = ((seat % 3) - 1) * 8,
            oy = (Math.floor(seat / 3) - 1) * 8;
        live.humans.set(p.userId, {
            participantId: p.id,
            userId: p.userId,
            arbId: HUMAN_ID_BASE + seat,
            name: displayName ?? 'Player',
            color: HUMAN_COLORS[seat % HUMAN_COLORS.length],
            seatIndex: seat,
            x: sx + ox,
            y: sy + oy,
            input: {},
    ...
    private async startRound(cfg: WerkConfig): Promise<void> {
        ...
        const botsEnabled =
            cfg.botMaxRealPlayers > 0
                ? realCount < cfg.botMaxRealPlayers &&
                  ((live.round.botRoster as unknown[])?.length ?? 0) > 0
                : false;
        ...
        for (const p of live.humans.keys()) {
            await this.participantRepo.update(
                { roundId: live.round.id, userId: p },
                { status: 'playing' },
            );
        }
        ...
        this.logger.log(
            `Werk round ${live.round.id} started  ${realCount} players, bots=${botsEnabled}`,
        );
    }
    ...
    private async cancelRoundRow(
        round: WerkRound,
        reason: string,
    ): Promise<void> {
        await this.dataSource.transaction(async (m) => {
            const r = await m.findOne(WerkRound, {
                where: { id: round.id },
            ...
            const parts = await m.find(WerkParticipant, {
                where: { roundId: r.id },
            });
            for (const p of parts) {
                ...
                const credit = await this.walletService.creditInSession(
                    {
                        userId: p.userId,
                        amountMinor: p.stakeMinor,
                        entryType: 'refund',
                        sourceType: 'werk_round',
                        sourceId: p.id,
                        idempotencyKey: `werk-refund:${p.id}`,
                        metadata: { reason },
                    },
                    m,
                );
    ...
    private async finalizeAndRotate(
        cfg: WerkConfig,
        playable: boolean,
    ): Promise<void> {
        if (this.live) {
            await this.roundRepo.update(
                { id: this.live.round.id },
                { activeGuard: null },
            );
            this.live = null;
        }
    ...
    private async settleRound(cfg: WerkConfig): Promise<void> {
        ...
        try {
            ...
            for (const h of humansArr) {
                gamesByUser.set(
                    h.userId,
                    await this.participantRepo.count({
                        where: { userId: h.userId, status: 'settled' },
                    }),
                );
            }
            ...
            const humans: RoundHuman[] = humansArr.map((h) => ({
                ...
                reachedCenter:
                    live.round.mode === 'B' ? inCenter(h.x, h.y) : true,
                gamesPlayed: gamesByUser.get(h.userId) ?? 0,
            }));
            const bots0: RoundBot[] = live.botSim.bots.map((b) => ({
                id: b.id,
            ...
            const payingRanks = Math.max(
                1,
                payMults.filter((m) => m > 0).length,
            );
            const poolTotal = live.layout.coins.reduce(
                (a, c) => a + c.value,
                0,
            );
            const partRows = await this.participantRepo.find({
                where: { roundId: live.round.id },
            });
            const stakeByPart = new Map(
                partRows.map((p) => [p.id, p.stakeMinor]),
            );

            const result = await this.dataSource.transaction(async (m) => {
                const round = await m.findOne(WerkRound, {
                    where: { id: live.round.id },
                    lock: { mode: 'pessimistic_write' },
                });
                if (!round || round.status === 'completed') return null;
                const lockedCfg = await m.findOne(WerkConfig, {
                    where: { key: 'default' },
                    lock: { mode: 'pessimistic_write' },
                });
                if (!lockedCfg)
                    throw new BadRequestException('Config unavailable');

                // Periodic forced-loss for larger (non-small) bot-enabled rounds.
                let periodicForceLose = false;
                const notSmall =
                    humans.length >= lockedCfg.houseGuaranteedBelowPlayers;
                if (
                    round.botsEnabled &&
                    lockedCfg.winControlEnabled &&
                    notSmall &&
                    lockedCfg.botForcedWinEveryNRounds > 0
                ) {
                    lockedCfg.winControlCounter =
                        (lockedCfg.winControlCounter ?? 0) + 1;
                    if (
                        lockedCfg.winControlCounter >=
                        lockedCfg.botForcedWinEveryNRounds
                    ) {
                ...
                const draw = await this.rngService.drawSeed({
                    gameType: 'werk',
                    gameReference: round.id,
                    metadata: { winControl: true },
                    manager: m,
                });
                ...
                const wc = applyRoundWinControl(rng, humans, bots0, {
                    ...
                    houseGuaranteedBelowPlayers:
                        lockedCfg.houseGuaranteedBelowPlayers,
                    periodicForceLose,
                });
                ...
                for (const hr of humansArr) {
                    ...
                    prizeByPart.set(
                        hr.participantId,
                        this.werkService.computePrize(
                            lockedCfg,
                            stakeByPart.get(hr.participantId) ?? 0,
                            rr.rank,
                            rr.tieCount,
                            !rr.eligible,
                        ),
                ...
                const standings = ranked.map((r) => {
                    if (r.isHuman) {
                        const hr = humansArr.find(
                            (h) => h.participantId === r.key,
                        )!;
                        return {
                            id: hr.seatIndex,
                            participantId: hr.participantId,
                            userId: hr.userId,
                            name: hr.name,
                            isHuman: true,
                            color: hr.color,
                            ...
                            eligible: r.eligible,
                            rank: r.rank,
                            prizeMinor: prizeByPart.get(hr.participantId) ?? 0,
                        };
                    }
                ...
                for (const hr of humansArr) {
                    ...
                    const p = await m.findOne(WerkParticipant, {
                        where: { id: hr.participantId },
                    });
                    if (!p) continue;
                    p.coinValue = hr.coinValue;
                    p.reachedCenter =
                        live.round.mode === 'B' ? inCenter(hr.x, hr.y) : true;
                    p.rank = rr.rank;
                    ...
                    p.settledAt = new Date();
                    p.resultJson = {
                        winControl: wc.perHuman[hr.participantId],
                    };
                    if (prize > 0) {
                        const credit = await this.walletService.creditInSession(
                            {
                                ...
                                sourceType: 'werk_round',
                                sourceId: p.id,
                                idempotencyKey: `werk-prize:${p.id}`,
                                metadata: {
                                    rank: rr.rank,
                                    tieCount: rr.tieCount,
                                    roundId: round.id,
                                },
                            },
                            m,
                        ...
                        p.walletCredit = {
                            ledgerEntryId: credit.ledgerEntry.id,
                        };
                    }
                    await m.save(p);
                }
                ...
                round.resultJson = {
                    ...
                    winControl: {
                        forced: wc.forced,
                        perHuman: wc.perHuman,
                        auditLogId: draw.auditLogId ?? null,
                    },
                };
            ...
            if (result) {
                ...
                this.gateway.emitWerkRoundCompleted(
            ...
            this.logger.error(
                `Werk settle failed: ${err instanceof Error ? err.message : err}`,
            );
    ...
    async join(
        ...
        const participant = await this.dataSource.transaction(async (m) => {
            ...
            const debit = await this.walletService.debitInSession(
                {
                    ...
                    sourceType: 'werk_round',
                    sourceId: p.id,
                    idempotencyKey: `werk-stake:${p.id}`,
                    metadata: { roundId: live.round.id },
                },
                m,
            );
            ...
            if (!live.round.scheduledStartAt) {
                live.round.scheduledStartAt = new Date(
                    Date.now() + (cfg.lobbyCountdownSec ?? 15) * 1000,
                );
    ...
    async leave(userId: string): Promise<{ left: true }> {
        ...
        if (live.round.status === 'lobby') {
            await this.dataSource.transaction(async (m) => {
                const p = await m.findOne(WerkParticipant, {
                    where: { roundId: live.round.id, userId },
                });
    ...
    handleInput(userId: string, raw: unknown): void {
        ...
        const input: WerkInput = {
            moveX:
                typeof r.moveX === 'number'
                    ? Math.max(-1, Math.min(1, r.moveX))
                    : 0,
            moveY:
                typeof r.moveY === 'number'
                    ? Math.max(-1, Math.min(1, r.moveY))
                    : 0,
            sprint: !!r.sprint,
            usePower: !!r.usePower,
        };
        if (
            this.leader &&
            this.live?.round.status === 'running' &&
            this.live.humans.has(userId)
        ) {
            this.applyInput(userId, input);
        } else {
            // Not the leader (or no live round here)  hand off to the instance that is.
            this.redis
                .publish(WERK_INPUT_CHANNEL, JSON.stringify({ userId, input }))
                .catch(() => undefined);
        }
    }
    ...
    private roundView(
        ...
        const live =
            this.live && this.live.round.id === round.id ? this.live : null;
        const you = userId && live ? live.humans.get(userId) : undefined;
        const timeLeft =
            round.status === 'running' && live
                ? Math.max(0, round.durationSec - live.elapsed)
                : round.status === 'lobby'
                  ? round.durationSec
                  : 0;
        const countdown =
            round.scheduledStartAt && round.status === 'lobby'
                ? Math.max(
                      0,
                      Math.ceil(
                          (round.scheduledStartAt.getTime() - Date.now()) /
                              1000,
                      ),
                  )
                : null;
        return {
            ...
            standings:
                (round.resultJson as { standings?: unknown[] } | null)
                    ?.standings ?? [],
        };
    }
