    ...
    implements OnApplicationBootstrap, OnApplicationShutdown
{
    ...
    async onApplicationBootstrap(): Promise<void> {
        ...
        if (!token) {
            this.logger.log(
                'TELEGRAM_AGENT_BOT_TOKEN is not set  agent bot will not start',
            );
            return;
        }
        ...
        if (!miniAppUrl) {
            this.logger.warn(
                'TELEGRAM_AGENT_MINIAPP_URL is not set  agent bot will not start',
            );
            return;
        }
        ...
        this.bot.catch((err) => {
            this.logger.error(
                `Error in agent bot middleware: ${err.message}`,
                err.stack,
            );
        });
        ...
        try {
            await this.bot.api.setMyCommands([
                { command: 'start', description: 'Link your agent account' },
                {
                    ...
                    description: 'Update your shared location',
                },
            ]);
            ...
            if (webhookUrl) {
                ...
                this.logger.log(
                    'TELEGRAM_AGENT_BOT_WEBHOOK_URL is not set  starting agent bot with long polling',
                );
                this.isPolling = true;
                this.bot
                    .start({
                        onStart: () =>
                            this.logger.log('Agent bot started (long polling)'),
                    })
                    .catch((err) => this.logger.error('Agent bot error', err));
            }
        } catch (error) {
            this.logger.error(
                'Failed to start agent bot',
    ...
    private registerCommands(bot: Bot, miniAppUrl: string): void {
        ...
        bot.on('message:contact', (ctx) => {
            ...
            this.deferTask(`agent:contact:${telegramUserId}`, async () => {
                ...
                try {
                    agent =
                        await this.usersService.findAgentByPhone(normalized);
                } catch (err) {
                    this.logger.error(
                        'Agent lookup failed',
                ...
                if (agent.status === 'suspended' || agent.status === 'closed') {
                    await ctx.reply(
                        'Your agent account is not active. Contact your admin.',
                    );
                    return;
                }
                ...
                try {
                    ...
                    this.logger.error(
                        `Failed to link Telegram identity for agent ${agent.id}`,
                        err instanceof Error ? err.stack : err,
                    );
                    await ctx.reply(
                ...
                try {
                    alreadyShared = await this.usersService.hasSharedLocation(
                        agent.id,
                    );
                } catch (err) {
                    this.logger.error(
                        `Failed to check shared location for agent ${agent.id}`,
                        err instanceof Error ? err.stack : err,
                    );
                }
                ...
                await ctx.reply(`Linked! Welcome, ${agent.displayName}.`, {
        ...
        bot.on('message:location', (ctx) => {
            ...
            this.deferTask(`agent:location:${telegramUserId}`, async () => {
                ...
                if (!agentId) {
                    await ctx.reply(
                        'Please share your phone number first to link your agent account.',
                        {
                            reply_markup: this.contactRequestKeyboard(),
                        },
                    );
                ...
                try {
                    await this.usersService.setAgentSharedLocation(
                        agentId,
                        latitude,
                        longitude,
                    );
                    ...
                    this.logger.error(
                        `Failed to save shared location for agent ${agentId}`,
                        err instanceof Error ? err.stack : err,
                    );
                    await ctx.reply(
        ...
        bot.command('updatelocation', (ctx) => {
            ...
            this.deferTask(
                `agent:updatelocation:${telegramUserId}`,
                async () => {
                    const agentId =
                        await this.resolveLinkedAgentId(telegramUserId);
                    if (!agentId) {
                        await ctx.reply(
                            'Please share your phone number first to link your agent account:',
                            {
                    ...
                    await ctx.reply(
                        'Share your current location to update it:',
                        {
                            reply_markup: this.locationRequestKeyboard(),
                        },
                    );
    ...
    private async resolveLinkedAgentId(
        ...
        try {
            const linked = await this.usersService.findAgentPhoneByTelegramId(
                String(telegramUserId),
            );
            if (!linked) return null;
            const agent = await this.usersService.findAgentByPhone(
                linked.phoneNumber,
            );
            if (agent?.id) {
                this.linkedAgentCache.set(telegramUserId, agent.id);
            ...
            this.logger.error(
                `Failed to resolve agent for Telegram user ${telegramUserId}`,
                err instanceof Error ? err.stack : err,
            );
            return null;
        }
