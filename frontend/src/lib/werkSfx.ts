...
function tone(freq: number, dur: number, gain: number, type: OscillatorType = 'sine', sweepTo?: number): void {
  const c = ac();
  if (!c) return;
...
function chord(freqs: number[], dur: number, gain: number, type: OscillatorType = 'triangle'): void {
...
export function victoryFanfare(): void {
  ...
  notes.forEach((f, i) => setTimeout(() => tone(f, 0.22, 0.16, 'triangle'), i * 130));
}
