  ...
  it('builds a legal 16-ball rack', () => {
    ...
    for (let i = 0; i < balls.length; i++) {
      for (let j = i + 1; j < balls.length; j++) {
        const d = Math.hypot(balls[i].pos.x - balls[j].pos.x, balls[i].pos.y - balls[j].pos.y);
...
describe('pool engine — simulation', () => {
  it('produces identical results for identical inputs (determinism)', () => {
    ...
    const input = { angle: 0.02, power: 0.95, spin: { side: 0.1, vertical: 0.2 } };
  ...
  it('does not mutate the caller\'s ball array', () => {
    ...
    runShot(balls, { angle: 0, power: 1, spin: { side: 0, vertical: 0 } }, table);
  ...
  it('a break scatters the rack and everything comes to rest inside the table', () => {
    ...
    const result = runShot(balls, { angle: 0, power: 1, spin: { side: 0, vertical: 0 } }, table);
    expect(result.steps).toBeLessThan(60_000); // terminated naturally, not the cap
    expect(result.events.length).toBeGreaterThan(0);

    const rackYs = rackEightBall(table, 999).filter((b) => b.number !== 0).map((b) => b.pos.y);
    const spread = Math.max(...result.balls.filter((b) => !b.pocketed && b.number !== 0).map((b) => b.pos.y));
    ...
    for (const b of result.balls) {
      ...
      expect(b.pos.x).toBeLessThanOrEqual(table.width + table.pocketRadius);
      expect(b.pos.y).toBeGreaterThanOrEqual(-table.pocketRadius);
      expect(b.pos.y).toBeLessThanOrEqual(table.height + table.pocketRadius);
    }
  });
  ...
  it('sinks an object ball lined up with a corner pocket', () => {
    ...
    const result = runShot(balls, { angle: Math.atan2(dir.y, dir.x), power: 0.55, spin: { side: 0, vertical: 0 } }, table);
  ...
  it('draw makes the cue recoil while follow carries it forward', () => {
    const setup = (): Ball[] => [cueBall(0.5, table.height / 2), objectBall(5, 1.0, table.height / 2)];
    const straight = { angle: 0, power: 0.5 };

    const draw = runShot(setup(), { ...straight, spin: { side: 0, vertical: -1 } }, table);
    const follow = runShot(setup(), { ...straight, spin: { side: 0, vertical: 1 } }, table);
