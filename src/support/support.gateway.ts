...
export class SupportGateway implements OnGatewayConnection {
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      client.data.isAgent =
        Array.isArray(user.roles) && (user.roles.includes('agent') || user.roles.includes('admin'));

      await client.join(`support_user_${payload.sub}`);
      if (client.data.isAgent) {
        await client.join('support_agents');
      }
      this.logger.debug(`Support socket connected: ${client.id} (user ${payload.sub})`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.logger.warn(`Unauthorized support socket: ${client.id} - ${message}`);
      client.disconnect();
    }
  }
  ...
  async handleOpenTicket(client: Socket, payload: { ticketId: string }) {
    if (payload?.ticketId) await client.join(`support_ticket_${payload.ticketId}`);
  }

  @SubscribeMessage('support.ticket.leave')
  async handleLeaveTicket(client: Socket, payload: { ticketId: string }) {
    if (payload?.ticketId) await client.leave(`support_ticket_${payload.ticketId}`);
  }

  /** Live chat send. Persists to a `live_chat` ticket, then fans the message out. */
  @SubscribeMessage('support.chat.send')
  async handleChatSend(client: Socket, payload: { ticketId?: string; text: string }) {
    ...
    if (!isAgent) {
      this.server.to('support_agents').emit('support.livechat.activity', { ticketId, userId });
    }
  }

  @SubscribeMessage('support.typing')
  handleTyping(client: Socket, payload: { ticketId: string; typing: boolean }) {
  ...
  emitSupportTicketCreated(payload: SupportTicketCreatedPayload): void {
    this.server?.to('support_agents').emit('support.ticket.created', payload);
  }

  emitSupportMessage(ticketId: string, payload: SupportMessagePayload): void {
    this.server?.to(`support_ticket_${ticketId}`).emit('support.message.new', payload);
