...
export class AdminBootstrapService implements OnApplicationBootstrap {
    ...
    async onApplicationBootstrap(): Promise<void> {
        const raw = this.configService
            .get<string>('ADMIN_BOOTSTRAP_ACCOUNTS')
            ?.trim();
        if (!raw) return;
        ...
        try {
            ...
            if (!Array.isArray(parsed))
                throw new Error(
                    'ADMIN_BOOTSTRAP_ACCOUNTS must be a JSON array',
                );
        ...
        for (const account of accounts) {
            if (!account?.phoneNumber || !account?.password) {
                this.logger.warn(
                    'Skipping an ADMIN_BOOTSTRAP_ACCOUNTS entry missing phoneNumber/password',
                );
                continue;
            }
            if (account.password.length < 8) {
                this.logger.warn(
                    `Skipping admin bootstrap for ${account.phoneNumber}  password must be at least 8 characters`,
                );
                continue;
            }
            try {
                ...
                if (result === 'created') {
                    this.logger.log(
                        `Bootstrapped admin account for ${account.phoneNumber}`,
                    );
                }
            } catch (err) {
                this.logger.error(
