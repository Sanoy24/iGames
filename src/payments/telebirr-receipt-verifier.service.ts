...
export class TelebirrReceiptVerifierService {
    ...
    async verifyReceipt(
        ...
        try {
            ...
            if (!(error instanceof ServiceUnavailableException)) {
                ...
                try {
                    ...
                    if (creditedName) {
                        ...
                        const matchingAgent = agents.find(
                            (agent) =>
                                this.normalize(agent.displayName) ===
                                normalizedCreditedName,
                        );
                        if (matchingAgent) {
                ...
                await this.telebirrDepositRepository
                    .save(deposit)
                    .catch(() => {});
            }
            throw error;
    ...
    async verifyParsedReceipt(
        ...
        const amountBirr =
            parsedReceipt.settled_amount ?? parsedReceipt.total_amount;
        if (
        ...
        if (!parsedReceipt.date) {
            throw new BadRequestException(
                'Telebirr receipt is missing transaction date',
            );
        }
        const txDate = this.parseTelebirrDate(parsedReceipt.date);
        if (!txDate) {
            throw new BadRequestException(
                'Telebirr receipt transaction date is invalid',
            );
        }
        const now = new Date();
        ...
        if (diffMs > MAX_AGE_MS) {
            throw new BadRequestException(
                'Telebirr transaction is too old (exceeds 30 minutes)',
            );
        }
        if (diffMs < -5 * 60 * 1000) {
            throw new BadRequestException(
                'Telebirr transaction is in the future',
            );
        }
        ...
        if (!creditedName) {
            throw new BadRequestException(
                'Telebirr receipt is missing credited party (receiver) name',
            );
        }
        ...
        const matchingAgent = agents.find(
            (agent) =>
                this.normalize(agent.displayName) === normalizedCreditedName,
        );
    ...
    private async loadReceipt(receiptNo: string): Promise<string> {
        ...
        try {
            if (proxyUrl) {
                const html = await this.loadReceiptViaProxy(
                    proxyUrl.replace(/\/+$/, ''),
                    receiptNo,
                );
                this.logger.log(
                    `Receipt loaded via PROXY in ${Date.now() - started}ms (${html.length} bytes)`,
                );
                return html;
            }
            this.logger.warn(
                'TELEBIRR_PROXY_URL is not set  fetching Ethiotelecom DIRECTLY',
            );
            const html = await telebirrReceipt.utils.loadReceipt({ receiptNo });
            this.logger.log(
                `Receipt loaded DIRECT in ${Date.now() - started}ms`,
            );
            return html;
        } catch (error) {
            this.logger.error(
    ...
    private async toCreditMinor(amountBirr: number): Promise<number> {
        ...
        if (!Number.isSafeInteger(amountMinor) || amountMinor <= 0) {
            throw new BadRequestException(
                'Telebirr converted wallet amount is invalid',
            );
        }
        return amountMinor;
    }
