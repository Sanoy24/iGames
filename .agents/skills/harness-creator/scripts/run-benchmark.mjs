...
const output = path.resolve(
    args.output || path.join(target, 'harness-benchmark.json'),
);
const evalPath = path.resolve(
    args.evals || path.join(skillRoot, 'evals', 'evals.json'),
);
...
if (!selfCheck.skipped) {
    console.log(
        `Self-check: ${selfCheck.pass ? 'PASS' : 'FAIL'}  scaffolded harness scored ${selfCheck.score}/100`,
    );
    if (!selfCheck.pass && selfCheck.error) console.log(`  ${selfCheck.error}`);
}
console.log(formatScoreReport(harnessResult, target));
console.log(
    `Eval coverage: ${evalResult.score}/100 (${evalResult.passed}/${evalResult.total})`,
);
...
async function runSelfCheck() {
    ...
    try {
        ...
        await execFileAsync('node', [
            path.join(scriptDir, 'create-harness.mjs'),
            '--target',
            dir,
        ]);
...
function scoreEvals(evalsJson) {
    ...
    const checks = [];
    checks.push({
        pass: cases.length >= 10,
        message: 'At least 10 eval cases',
    });
    checks.push({
        pass: cases.some((item) => /minimal|creation/i.test(item.name)),
        message: 'Covers minimal harness creation',
    });
    checks.push({
        pass: cases.some((item) => /session|continuity/i.test(item.name)),
        message: 'Covers session continuity',
    });
    checks.push({
        pass: cases.some((item) => /assessment|score/i.test(item.name)),
        message: 'Covers harness assessment',
    });
    checks.push({
        pass: cases.some((item) => /verification/i.test(item.name)),
        message: 'Covers verification workflow',
    });
    checks.push({
        pass: cases.some((item) => /memory/i.test(item.name)),
        message: 'Covers memory taxonomy',
    });
    checks.push({
        pass: cases.some((item) => /tool|permission|safety/i.test(item.name)),
        message: 'Covers tool safety',
    });
    checks.push({
        pass: cases.some((item) =>
            /multi-agent|delegation|coordination/i.test(item.name),
        ),
    ...
    checks.push({
        pass: cases.every((item) => item.expectations?.length >= 3),
...
function renderBenchmarkHtml(report) {
    ...
    const evalHtml = htmlReport(
        report.harness,
        `Harness Benchmark: ${path.basename(report.target)}`,
    ).replace(
        '</main>',
        `${selfCheckSection}<section>
