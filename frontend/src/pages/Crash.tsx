...
const multAt = (elapsedMs: number) =>
    Math.max(100, Math.floor(100 * Math.exp(GROWTH_K * elapsedMs)));
...
export function Crash({ onBack }: { onBack: () => void }) {
    ...
    useEffect(() => {
        ...
        const onBetPublic = (e: {
            ...
            const bet: LiveBet = {
                betId: e.betId,
                name: e.name,
                stakeMinor: e.stakeMinor,
                autoCashoutX100: e.autoCashoutX100 ?? null,
                cashedOutAtX100: null,
                payoutMinor: 0,
                status: 'active',
            };
            setLiveBets((prev) =>
                prev.some((b) => b.betId === e.betId)
                    ? prev
                    : [bet, ...prev].slice(0, 80),
            );
        };
        const onCashoutPublic = (e: {
            betId: string;
            ...
            setLiveBets((prev) =>
                prev.map((b) =>
                    b.betId === e.betId
                        ? {
                              ...b,
                              cashedOutAtX100: e.cashedOutAtX100,
        ...
        const id = setInterval(async () => {
            try {
                ...
                if (active.status === 'running' && p !== 'running') {
                    clearTimer();
                    const startedAt = (active as { startedAt?: string | null })
                        .startedAt;
                    ...
                    setPhase('waiting');
                    startWaitingTimer(
                        configRef.current?.waitingDurationSeconds ?? 12,
                    );
                }
            } catch {
        ...
        const frame = () => {
            ...
            if (start != null) {
                ...
                if (elapsed - last.x >= 40) {
                    ...
                    if (hist.length > 320) {
                        histRef.current = hist.filter(
                            (_, i) =>
                                i === 0 || i % 2 === 0 || i === hist.length - 1,
                        );
                    }
                }
    ...
    const handlePlaceBet = async () => {
        ...
        if (!round) {
            setBetError(t('crash.noActiveRound'));
            return;
        }
        ...
        try {
            ...
            const bet = await crashApi.placeBet(
                round.id,
                stakeMinor,
                key,
                acVal,
            ...
            const raw = (e as { response?: { data?: { message?: unknown } } })
    ...
    const handleCashOut = async () => {
        ...
        try {
            ...
            myBetRef.current = bet;
            setCashoutResult({
                mx100: bet.cashedOutAtX100 ?? multiplierX100,
                payoutMinor: bet.payoutMinor,
            });
            walletApi
                .getWallet()
    ...
    const adjustStake = (delta: number) =>
        setStake(String(Math.max(minBet, Math.min(maxBet, stakeNum + delta))));
    const addStake = (v: number) =>
        setStake(String(Math.min(maxBet, stakeNum + v)));
    const potentialWin = myBet
        ? Math.floor((myBet.stakeMinor * multiplierX100) / 100)
        : 0;
    const canBet =
        phase === 'waiting' &&
        !myBet &&
                ...
                <div className='crash-stage'>
                    ...
                    {(phase === 'running' || phase === 'crashed') && (
                        ...
                        >
                            ...
                            {svgArea && (
                                <path d={svgArea} fill='url(#cg-fill)' />
                            )}
                            {svgPath && (
                                <path
                                    d={svgPath}
                    ...
                    >
                        ...
                        {phase === 'waiting' && (
                            ...
                            >
                                ...
                                <div className='crash-loading-bar'>
                                    <span />
                                </div>
                                <div
                                    style={{
                ...
                <AnimatePresence>
                    {cashoutResult && (
                        ...
                        >
                            ...
                            >
                                <Zap size={14} />
                                {t('crash.cashedOutWon', {
                                    mult: fmtMult(cashoutResult.mx100),
                                    amount: cashoutResult.payoutMinor,
                                })}
                            </div>
                        </motion.div>
                    )}
                            ...
                            {phase === 'waiting' && !myBet && (
                                ...
                                >
                                    ...
                                    >
                                        <button
                                            type='button'
                                            className='crash-step'
                                            aria-label='decrease'
                                            onClick={() =>
                                                adjustStake(-stepAmount)
                                            }
                                        >
                                            −
                                        </button>
                                ...
                                <motion.button
                                    ...
                                    style={{
                                        ...
                                        cursor: isCashingOut
                                            ? 'not-allowed'
                                            : 'pointer',
                                        opacity: isCashingOut ? 0.7 : 1,
                                    }}
                                    animate={{
                                        boxShadow: [
                                            '0 0 0 0 rgba(245,158,11,0)',
                                            '0 0 22px 6px rgba(245,158,11,0.4)',
                                            '0 0 0 0 rgba(245,158,11,0)',
                                        ],
                                    }}
                                    transition={{
