...
export function LocationPrompt() {
    ...
    const useMyLocation = () => {
        if (!navigator.geolocation) {
            addToast(
                'error',
                'Location is not available on this device  pick from the list instead.',
            );
            return;
        }
        setLocating(true);
        navigator.geolocation.getCurrentPosition(
            async (pos) => {
                try {
                    const match = await agentMatchApi.attempt(
                        pos.coords.latitude,
                    ...
                    if (match) {
                        ...
                        addToast(
                            'success',
                            `Connected with ${match.assignedAgentName ?? 'an agent'} nearby.`,
                        );
                    } else {
                        addToast(
                            'error',
                            'No on-duty agent nearby  pick one from the list instead.',
                        );
                    }
                } catch (e) {
                    addToast('error', getErrorMessage(e));
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    It is optional, but connecting with a nearby agent makes
