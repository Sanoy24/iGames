...
export class MpesaReceiptVerifierService {
    ...
    async verifySms(
        ...
        const existing = await this.mpesaDepositRepository.findOneBy({
        ...
        try {
            ...
            await this.saveRejected(parsed, userId, error).catch(
                () => undefined,
            );
            throw error;
        }
    ...
    async verifyParsed(parsed: ParsedMpesaSms): Promise<VerifiedMpesaReceipt> {
        ...
        if (parsed.direction !== 'sent' && parsed.direction !== 'paid') {
            throw new BadRequestException(
                'Could not tell from the SMS that money was sent to an agent',
            );
        }
        ...
        if (!parsed.transactedAt) {
            throw new BadRequestException(
                'M-PESA SMS is missing a readable transaction time',
            );
        }
        const freshness = checkFreshness(parsed.transactedAt);
        if (freshness === 'too_old') {
            throw new BadRequestException(
                'M-PESA transaction is too old (exceeds 30 minutes)',
            );
        }
        if (freshness === 'future') {
            throw new BadRequestException(
                'M-PESA transaction time is in the future',
            );
        }
        ...
        if (!creditedName) {
            throw new BadRequestException(
                'M-PESA SMS is missing the receiving agent name',
            );
        }
        ...
        const matchingAgent = agents.find(
            (agent) => normalizeName(agent.displayName) === normalizedName,
        );

        if (!matchingAgent) {
            throw new BadRequestException(
                `No active agent found with name matching "${creditedName}"`,
            );
        }
        if (
            matchingAgent.agentPermissions &&
            matchingAgent.agentPermissions.deposit === false
        ) {
            throw new BadRequestException(
                `Agent "${matchingAgent.displayName}" does not have deposit permission`,
            );
        }
        ...
        const receiverAccountMatched = accountMatchesPhone(
        ...
        return {
            ...
            verification: {
                ...
                receiverAccountMatched: portal.checked
                    ? (portal.receiverMatched ?? receiverAccountMatched)
    ...
    private async crossCheckPortal(
        ...
        let receipt;
        try {
            receipt = await this.receiptClient.fetchParsed(
            ...
            if (error instanceof BadRequestException) {
                throw new BadRequestException(
                    `M-PESA portal could not confirm this receipt: ${error.message}`,
                );
            }
        ...
        const amountMatched =
            receipt.amountBirr != null &&
            Math.abs(receipt.amountBirr - parsed.amountBirr) < 0.005;
    ...
    private async saveRejected(
        ...
        let amountMinor = 0;
        try {
            if (parsed.amountBirr > 0)
                amountMinor = await this.toCreditMinor(parsed.amountBirr);
        } catch {
        ...
        if (parsed.counterpartyName) {
            ...
            const agents = await this.userRepository
                .createQueryBuilder('user')
                .where('user.status = :status', { status: 'active' })
                .andWhere('JSON_CONTAINS(user.roles, :role)', {
                    role: '"agent"',
                })
                .getMany();
            matchedAgentId = agents.find(
                (a) => normalizeName(a.displayName) === normalized,
            )?.id;
        }
    ...
    private async toCreditMinor(amountBirr: number): Promise<number> {
        ...
        if (!Number.isSafeInteger(amountMinor) || amountMinor <= 0) {
            throw new BadRequestException(
                'M-PESA converted wallet amount is invalid',
            );
        }
        return amountMinor;
    }
