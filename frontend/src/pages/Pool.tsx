...
import {
    ...
    usePoolMatchSocket,
    sendPoolEmote,
    type PoolShotResolvedEvent,
    type PoolEmoteEvent,
} from '../hooks/usePoolSocket';
    ...
    return (
        <span
            style={{
                ...
                background: stripe ? '#f2ecdf' : color,
                overflow: 'hidden',
...
function PocketedTray({
    ...
    const ofGroup = (g: PoolGroup | null) =>
    ...
    const neutral =
        myGroup || oppGroup
            ? []
            : [...solids, ...stripes].sort((a, b) => a - b);

    const chips = (balls: number[]) => (
        <div style={{ display: 'flex', gap: 3, alignItems: 'center' }}>
            {balls.map((n) => (
                <BallChip key={n} n={n} size={size} />
            ))}
        </div>
    );
    return (
...
function MatchHeader({
    ...
    const Group = ({ g }: { g: PoolGroup | null }) =>
        !g ? null : (
            <span
                style={{
                    fontSize: 8.5,
...
function QuickChat({
    ...
    return (
        <div style={{ position: 'fixed', right: 14, bottom, zIndex: 2500 }}>
            ...
            <button
                onClick={() => setOpen((o) => !o)}
                aria-label={t('pool.quickChat')}
                style={{
                    width: 48,
                    height: 48,
                    borderRadius: '50%',
                    border: '1px solid var(--border, rgba(255,255,255,0.15))',
                    background: open ? '#243244' : '#1b2530',
                    color: '#fff',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    boxShadow: '0 8px 22px -6px rgba(0,0,0,0.75)',
                    cursor: 'pointer',
                }}
            >
...
export function Pool({ onBack }: { onBack: () => void }) {
    ...
    const seatLabel = (seat: Seat) =>
        seat === 'A' ? t('pool.player1') : t('pool.player2');
    ...
    const [emotes, setEmotes] = useState<
        Partial<Record<Seat, { id: string; ts: number }>>
    >({});
    ...
    usePoolMatchSocket(match?.id ?? null, {
        onMatchUpdated: (v) => {
            setMatch(v);
            if (v.status !== 'active')
                setEnded((e) => e ?? { winnerSeat: v.winnerSeat });
        },
        onShotResolved: (e: PoolShotResolvedEvent) => {
            setFeed((f) =>
                [`${seatLabel(e.seat)}: ${e.reason}`, ...f].slice(0, 6),
            );
            if (mySeat && e.seat !== mySeat)
                tableRef.current?.enqueueShot(e.input);
        },
        ...
        onEmote: (e: PoolEmoteEvent) => {
            ...
            const seat: Seat | null =
                e.userId === match?.seatAUserId
                    ? 'A'
                    : e.userId === match?.seatBUserId
                      ? 'B'
                      : null;
            if (!seat) return;
    ...
    const sendEmote = (id: string) => {
        if (match) sendPoolEmote(match.id, id);
    };
    ...
    if (match) {
        ...
        const myGroupLeft = myGroup
            ? match.board.filter(
                  (b) =>
                      ...
                      b.number !== 0 &&
                      b.number !== 8 &&
                      (myGroup === 'solids' ? b.number < 8 : b.number > 8),
              ).length
            : 1;
        ...
        const deadlineMs = match.turnDeadline
        ...
        const oppSeatName = mySeat
            ? mySeat === 'A'
                ? match.seatBName
                : match.seatAName
            : (match.seatBName ?? match.seatAName);
        const opponentName =
            match.mode === 'single'
                ? t('pool.opponentAI')
                : oppSeatName?.trim() || t('pool.opponent');
        const opponentThinking = match.status === 'active' && !isMyTurn;
        const oppGroup = mySeat
            ? match.groups[mySeat === 'A' ? 'B' : 'A']
            : null;
        ...
        const myEmote = emotes[meSeatKey]
            ? t(`pool.chat.${emotes[meSeatKey]!.id}`)
            : undefined;
        const oppEmote = emotes[oppSeatKey]
            ? t(`pool.chat.${emotes[oppSeatKey]!.id}`)
            : undefined;
        // Quick chat is a human-vs-human feature (no point taunting the AI).
        const showQuickChat =
            match.mode !== 'single' && match.status === 'active';
        const header = (
            <MatchHeader
                onBack={backToLobby}
            ...
            >
                <div
                    style={{
                        ...
                        animation:
                            'pool-pop 0.28s cubic-bezier(0.2,0.9,0.3,1.2)',
                    }}
                >
                    <div
                        ...
                        style={{
                            ...
                            color: ended.aborted
                                ...
                                : didWin
                                  ...
                                  : '#ff6b63',
                            textShadow: didWin
                                ? '0 2px 16px rgba(246,201,69,0.5)'
                                : 'none',
        ...
        if (landscape) {
            return (
                ...
                >
                    ...
                    {showQuickChat && (
                        <QuickChat onSend={sendEmote} bottom={16} />
                    )}
                    {resultModal}
                </div>
            );
    ...
    const searchElapsed = queueStartedAt
        ? Math.floor((now - queueStartedAt) / 1000)
        : 0;
        ...
        >
            ...
            {config?.singlePlayerEnabled !== false && (
                <LobbyCard
                    ...
                    subtitle={
                        config && config.singlePlayerStakeMinor > 0
                            ? t('pool.stakeAmount', {
                                  amount: config.singlePlayerStakeMinor,
                              })
                            : t('pool.freePractice')
                    }
                >
            ...
            {config?.twoPlayerEnabled !== false && (
                <LobbyCard
                    icon={<Swords size={22} />}
                    title={t('pool.ranked2p')}
                    subtitle={t('pool.rankedSubtitle')}
                >
                    {queuedStake != null ? (
                        noPlayers ? (
                            <div>
                            ...
                            >
                                ...
                                <div style={{ minWidth: 0 }}>
                                    ...
                                    >
                                        {t('pool.searchingCountdown', {
                                            secs: searchRemaining,
                                        })}
                                    </div>
                                </div>
                                <button
                    ...
                    {config && (
                        ...
                        >
                            {t('pool.stakesRange', {
                                min: config.minStakeMinor,
                                max: config.maxStakeMinor,
                                rake: config.rakePct,
                            })}
                        </div>
                    )}
...
function TournamentPanel({
    ...
    const myActive = matches.find(
        (m) =>
            m.status === 'active' &&
            (m.seatAUserId === myUserId || m.seatBUserId === myUserId),
    );

    // Localised tournament status (falls back to the raw value if unmapped).
    const statusLabel = (status: string) =>
        t(`pool.tStatus.${status}`, { defaultValue: status });

    return (
        <LobbyCard
            icon={<Trophy size={22} />}
            title={t('pool.tournaments')}
            subtitle={t('pool.tournamentsSubtitle')}
        >
            ...
            {tournament && (
                <div style={{ marginTop: 12, fontSize: 12.5 }}>
                    ...
                    <div style={{ color: 'var(--text-muted)' }}>
                        ...
                        {t('pool.tournamentPlayers', {
                            count: tournament.size,
                        })}{' '}
                        ·{' '}
                        {t('pool.tournamentPrize', {
                            amount: tournament.prizePoolMinor,
                        })}
                    </div>
                    {myActive && (
                        <button
                            className='btn btn-primary'
                            onClick={() => onPlay(myActive)}
