...
const SCREENSHOT_MIME_TYPES = RECEIPT_MIME_TYPES.filter(
    (t) => t !== 'application/pdf',
);
...
export class PaymentsController {
    ...
    @ApiOkResponse({
        description:
            'Active agent Telebirr deposit details, or null if nobody is on shift',
    })
    getActiveAgent() {
    ...
    @Get('active-agents')
    @ApiOkResponse({
        description: 'List of on-duty deposit agents the player may pick from',
    })
    getActiveAgents() {
    ...
    @ApiOkResponse({
        description:
            'Player-facing deposit rules (e.g. minimum deposit amount)',
    })
    getConfig() {
    ...
    @ApiQuery({
        name: 'receipt',
        required: false,
        description: 'Optional real receipt no to test full fetch+parse',
    })
    @ApiOkResponse({
        description: 'Egress diagnostics for the Telebirr receipt service',
    })
    ...
    @ApiOkResponse({
        description: 'Parsed receipt details  wallet is NOT credited',
    })
    ...
    @ApiOkResponse({
        description:
            'Parsed receipt details (from OCR + verified fetch)  wallet is NOT credited',
    })
    async previewTelebirrScreenshot(
        ...
        const receiptNo =
            await this.telebirrScreenshotOcrService.extractTransactionNumber(
                imageBuffer,
            );
        const preview = await this.paymentsService.previewTelebirrReceipt(
            user.id,
            { receiptNo },
        );
        return {
            ...preview,
            receiptNo,
            fileUrl: `${DEPOSIT_RECEIPT_SUBDIR}/${file.filename}`,
        };
    }
    ...
    @ApiOkResponse({
        schema: { example: { fileUrl: 'deposit-receipts/<uuid>.jpg' } },
    })
    ...
    @HttpCode(HttpStatus.OK)
    @ApiOkResponse({
        description: 'Parsed M-PESA SMS details  wallet is NOT credited',
    })
    previewMpesaSms(
        @CurrentUser() user: AuthenticatedUser,
