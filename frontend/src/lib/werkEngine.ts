...
export const COIN_AM: Record<CoinType, string> = {
    bronze: 'ነሃስ ሳንቲም',
    silver: 'ብር ሳንቲም',
    gold: 'ወርቅ ሳንቲም',
};
...
export class WerkGame {
    ...
    constructor(opts: WerkRoundOpts) {
        ...
        if (opts.yourSeat != null)
            this.setOwnSeat(opts.yourSeat, opts.yourName);
        if (this.human) {
            this.serverX = this.human.x;
            this.serverY = this.human.y;
        }
    }
    ...
    get coins(): RenderCoin[] {
        return this.layout.coins.map((c) => ({
            ...c,
            collected: this.taken.has(c.index) || this.myCollected.has(c.index),
        }));
    }

    get powerups(): RenderPowerup[] {
        return this.layout.powerups.map((p, i) => ({
            ...p,
            taken: this.takenPowerups.has(i),
        }));
    }
    ...
    setOwnSeat(seat: number | null, name?: string) {
        ...
        if (!this.human)
            this.human = this.makeHuman(seat, name ?? this.opts.yourName);
    }

    applyRoundState(view: WerkRoundView) {
        this.status = view.status;
        if (typeof view.timeLeft === 'number') this.timeLeft = view.timeLeft;
        if (view.yourSeat != null && !this.human)
            this.setOwnSeat(view.yourSeat);
    ...
    applySnapshot(snap: WerkSnapshot) {
        ...
        if (
            this.opts.mode === 'B' &&
            !this.warnedSprint &&
            this.timeLeft <= this.opts.finalSprintWarningSec
        ) {
    ...
    private buildInput(input: HumanInput): WerkInputMsg {
        ...
        return {
            ...
            moveY: my,
            sprint: !!input.sprint,
            usePower: !!input.usePower,
        };
    }

    private consumePower() {
        const h = this.human!;
        if (h.boost <= 0 && h._pendingSpeed) {
            h.boost = SPEED_BOOST_SEC;
            h._pendingSpeed = false;
        } else if (h.magnet <= 0 && h._pendingMagnet) {
            h.magnet = MAGNET_SEC;
            h._pendingMagnet = false;
        } else if (h.shield <= 0 && h._pendingShield) {
            h.shield = SHIELD_SEC;
            h._pendingShield = false;
        }
    }
    ...
    private collectHuman() {
        ...
        for (const c of this.layout.coins) {
            if (this.myCollected.has(c.index) || this.taken.has(c.index))
                continue;
