...
export type WerkBotPersonality =
    | 'gatherer'
    | 'sniper'
    | 'strategist'
    | 'explorer'
...
const WERK_CONFIG_EDITABLE_KEYS = [
    ...
    'botForcedWinEveryNRounds',
    'onboardingWinControlEnabled',
    'onboardingBotWinGames',
    'onboardingUserWinGames',
] as const satisfies readonly (keyof AdminWerkConfig)[];

/** Keep only editable, non-null fields so the whitelist pipe never rejects. */
function cleanWerkConfig(
    dto: Partial<AdminWerkConfig>,
): Partial<AdminWerkConfig> {
...
export const adminWerkApi = {
    getConfig: () =>
        api.get<AdminWerkConfig>('/admin/werk/config').then((r) => r.data),
    updateConfig: (dto: Partial<AdminWerkConfig>) =>
        api
            .patch<AdminWerkConfig>('/admin/werk/config', cleanWerkConfig(dto))
            .then((r) => r.data),
    // Bot pool management.
    listBots: () =>
        api.get<AdminWerkBot[]>('/admin/werk/bots').then((r) => r.data),
    ...
    updateBot: (id: number, dto: Partial<CreateWerkBotInput>) =>
        api
            .patch<AdminWerkBot>(`/admin/werk/bots/${id}`, dto)
            .then((r) => r.data),
    deleteBot: (id: number) =>
        api
            .delete<{ deleted: true }>(`/admin/werk/bots/${id}`)
            .then((r) => r.data),
};
