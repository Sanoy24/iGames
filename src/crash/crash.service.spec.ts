...
function makeService({
    ...
    const mockRoundRepo = {
        ...
        save: jest
            ...
            .mockImplementation((r) =>
                Promise.resolve({ id: 'round-new', ...r }),
            ),
    ...
    const mockRngService = {
        drawUniqueNumbers: jest
            .fn()
            .mockResolvedValue({ numbers: [500_001], algorithmVersion: 1 }),
    };
    ...
    const service = new CrashService(
        ...
        {
            findOne: jest.fn().mockResolvedValue({ displayName: 'Player' }),
        } as any,
        {
            emitCrashBetPublic: jest.fn(),
            emitCrashCashoutPublic: jest.fn(),
        } as any,
    );
...
describe('CrashService  unit (mocked repos)', () => {
    ...
    describe('abandonStaleRounds', () => {
        ...
        it('credits a refund for each active bet in the stale round', async () => {
            ...
            const { service, mockWalletService } = makeService({
                rounds: [round],
                bets: [bet1, bet2],
            });
        ...
        it('refund uses idempotency key crash-abandon-refund:<betId>', async () => {
            ...
            expect(mockWalletService.creditInSession).toHaveBeenCalledWith(
                expect.objectContaining({
                    idempotencyKey: 'crash-abandon-refund:bet-99',
                }),
                expect.anything(),
            );
        });
