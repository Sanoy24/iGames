...
export class AuthService {
     ...
     */
    async resolveAgentPhoneFromTelegram(
        initData: string,
    ): Promise<{ phoneNumber: string; displayName: string } | null> {
        const agentBotToken = this.configService.get<string>(
        ...
        const validated =
            this.telegramMiniAppAuthService.validateInitDataAgainstToken(
                initData,
                agentBotToken,
            );
        return this.usersService.findAgentPhoneByTelegramId(
    ...
    async loginWithTelegramMiniApp(
        ...
        const validatedTelegramData =
            this.telegramMiniAppAuthService.validateInitData(initData);
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            if (!user.phoneNumber) {
                throw new ForbiddenException({
                    code: 'PHONE_REQUIRED',
                    message:
                        'Please open our Telegram bot and share your phone number to continue.',
                });
            }
            ...
            const alreadyGranted = !!(
                user.productMetadata as Record<string, unknown> | undefined
            )?.welcomeBonusGranted;
            if (!alreadyGranted) {
    ...
    async loginWithCredentials(
        ...
        const user = await this.usersService.findBackofficeUserByCredentials(
            phoneNumber,
            password,
        );
    ...
    async refreshTokens(rawRefreshToken: string): Promise<AuthTokenResponse> {
        ...
        try {
            payload = await this.jwtService.verifyAsync<JwtSubjectPayload>(
                rawRefreshToken,
                {
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            const refreshSession = await refreshSessionRepo.findOneBy({
