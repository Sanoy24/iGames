...
export class WithdrawalProofVerifierService {
  ...
  private async verifyTelebirr(input: {
    ...
    if (!reference) throw new BadRequestException('Telebirr receipt number was not found');

    if (!this.telebirrClient.isAcceptedTransactionStatus(parsed.transaction_status)) {
    ...
    const receiverMatched = accountMatchesPhone(receiverAccount, input.destinationAccount);
  ...
  private async verifyMpesa(input: {
    ...
    const receiverMatched = accountMatchesPhone(parsed.counterpartyPhone, input.destinationAccount);
    if (receiverMatched === false) {
      throw new BadRequestException(
  ...
  private assertFresh(txDate: Date | null): void {
    if (!txDate) {
      throw new BadRequestException('The payout proof is missing a readable transaction time');
    }
    const verdict = checkFreshness(txDate, new Date(), WithdrawalProofVerifierService.MAX_AGE_MS);
    if (verdict === 'too_old') {
      throw new BadRequestException('The payout transaction is too old (exceeds 60 minutes)');
    }
    if (verdict === 'future') {
      throw new BadRequestException('The payout transaction time is in the future');
    }
  }
}
