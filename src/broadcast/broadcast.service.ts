...
export class BroadcastService {
  ...
  async list(): Promise<BroadcastMessage[]> {
    return this.broadcastRepo.find({ order: { createdAt: 'DESC' }, take: 100 });
  }
  ...
  async create(dto: CreateBroadcastDto, adminId?: string): Promise<BroadcastMessage> {
    const hasText = !!dto.text && dto.text.trim().length > 0;
    const imagePath = dto.imageFilename ? this.resolveStoredImagePath(dto.imageFilename) : null;
    if (!hasText && !imagePath) {
      throw new BadRequestException('A broadcast needs a message, an image, or both');
    }
    if (imagePath && hasText && (dto.text as string).length > TELEGRAM_CAPTION_MAX) {
      throw new BadRequestException(`With an image attached, the caption must be ${TELEGRAM_CAPTION_MAX} characters or fewer`);
    }
    ...
    const row = this.broadcastRepo.create({
      ...
      recurrence: dto.scheduleType === 'recurring' ? (dto.recurrence as BroadcastRecurrence) : null,
    ...
    if (dto.scheduleType === 'once') {
      row.nextRunAt = this.computeOnceRunAt(dto.scheduledAtLocal as string, offset);
      if (row.nextRunAt.getTime() <= Date.now()) {
        throw new BadRequestException('Scheduled time must be in the future');
      }
    } else {
      row.nextRunAt = this.computeNextRecurrence(dto.recurrence as BroadcastRecurrence, offset, new Date());
    }
  ...
  async sendNow(id: string): Promise<BroadcastMessage> {
    ...
    if (!['draft', 'scheduled', 'sent', 'failed', 'cancelled'].includes(row.status)) {
      throw new BadRequestException(`Cannot send a broadcast in status "${row.status}"`);
    }
    const claimed = await this.broadcastRepo.update(
      { id, status: row.status },
      { status: 'sending', lastRunAt: new Date() },
    );
    if (!claimed.affected) throw new BadRequestException('Broadcast is already being sent');
  ...
  async cancel(id: string): Promise<BroadcastMessage> {
    ...
    throw new BadRequestException(`Cannot cancel a broadcast in status "${row.status}"`);
  }

  async remove(id: string): Promise<{ deleted: true }> {
    const row = await this.get(id);
    if (row.status === 'sending') throw new BadRequestException('Stop the broadcast before deleting it');
  ...
  private async recoverStaleSending(): Promise<void> {
    ...
    for (const row of stale) {
      ...
      if (row.scheduleType === 'recurring' && row.recurrence) {
        row.status = 'scheduled';
        row.nextRunAt = this.computeNextRecurrence(row.recurrence, row.timezoneOffsetMinutes, new Date());
      } else {
        row.status = 'failed';
      }
      row.lastError = 'Interrupted before completion (recovered by scheduler)';
  ...
  private async dispatch(id: string): Promise<void> {
    ...
    try {
      ...
      const imageAbsolutePath = row.imagePath ? join(UPLOADS_ROOT, row.imagePath) : null;
      const { sent, failed, lastError } = await this.telegramBotService.sendBroadcastMessage({
        chatIds,
        text: row.text,
        imageAbsolutePath,
        buttons: row.buttons,
        ...
        onProgress: (s, f) => {
          void this.broadcastRepo.update(id, { sentCount: s, failedCount: f });
        },
      });
      ...
      if (fresh) await this.finalize(fresh, fresh.sentCount, fresh.failedCount, message);
    }
  }
  ...
  private async finalize(
    ...
    await this.broadcastRepo.save(row);
    this.logger.log(`Broadcast ${row.id} finished: ${sent} sent, ${failed} failed, status=${row.status}`);
  }
  ...
  resolveStoredImagePath(filename: string): string {
    if (!/^[a-zA-Z0-9._-]+$/.test(filename)) throw new BadRequestException('Invalid image reference');
    const relative = `${BROADCAST_IMAGE_SUBDIR}/${filename}`;
    if (!existsSync(join(UPLOADS_ROOT, relative))) {
      throw new BadRequestException('Uploaded image not found — please re-upload');
    }
    return relative;
  }
  ...
  private computeNextRecurrence(rec: BroadcastRecurrence, offsetMinutes: number, from: Date): Date {
    ...
    if (rec.frequency === 'weekly') {
      ...
      let ms = Date.UTC(ly, lmo, ld + daysAhead, h, mi) - offsetMinutes * 60_000;
      if (ms <= from.getTime()) {
        daysAhead += 7;
        ms = Date.UTC(ly, lmo, ld + daysAhead, h, mi) - offsetMinutes * 60_000;
      }
      return new Date(ms);
    }
    ...
    if (ms <= from.getTime()) ms = Date.UTC(ly, lmo, ld + 1, h, mi) - offsetMinutes * 60_000;
    return new Date(ms);
  }
}
