...
export class MpesaReceiptClientService {
    ...
    if (!success || !envelope.base64Data) {
      throw new BadRequestException(
        `M-PESA receipt ${code} was not found or not successful` +
          (envelope.responseDescription ? ` (${envelope.responseDescription})` : ''),
      );
    }

    let text: string;
    try {
      const pdf = await pdfParse(Buffer.from(envelope.base64Data, 'base64'));
      text = pdf.text ?? '';
    } catch (error) {
  ...
  extractCode(codeOrUrl: string): string {
    ...
    if (!trimmed) throw new BadRequestException('M-PESA receipt code is required');
    ...
    if (urlMatch) {
      ...
      if (url.hostname !== MPESA_RECEIPT_HOST) {
        throw new BadRequestException('M-PESA receipt URL host is not allowed');
      }
      const code = url.pathname.split('/').filter(Boolean).at(-1);
      if (!code) throw new BadRequestException('M-PESA receipt URL does not include a code');
  ...
  private async loadReceipt(code: string): Promise<ReceiptApiResponse> {
    const proxyUrl = this.configService.get<string>('MPESA_PORTAL_URL')?.trim();
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`M-PESA portal returned HTTP ${response.status}`);
      }
      const data = (await response.json()) as ReceiptApiResponse;
      this.logger.log(`M-PESA receipt ${code} loaded (${proxyUrl ? 'proxy' : 'direct'}) in ${Date.now() - started}ms`);
      return data;
    } catch (error) {
      this.logger.error(
      ...
      throw new ServiceUnavailableException({
        ...
        detail: error instanceof Error ? error.message : 'Unknown M-PESA receipt error',
      });
    }
  }
