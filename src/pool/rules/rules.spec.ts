...
const contact = (n: number, t = 0.2): ShotEvent => ({
    type: 'ball-ball',
    t,
    ballNumber: 0,
    otherNumber: n,
});
const rail = (n: number, t = 0.4): ShotEvent => ({
    type: 'ball-cushion',
    t,
    ballNumber: n,
});
...
describe('rules  the break', () => {
    ...
    it('legal break by rails with no pot passes the turn', () => {
        ...
        const out = resolveShot(
            pre,
            makeResult({ balls: board, firstContactNumber: 1, events }),
            table,
        );
        expect(out.fouls).toEqual([]);
...
describe('rules  assigned play', () => {
    ...
    it('missing everything is a no-contact foul', () => {
        ...
        const out = resolveShot(
            pre,
            makeResult({ balls: board, firstContactNumber: null, events: [] }),
            table,
        );
    ...
    it('scratching in play is a foul with ball in hand and cue re-spotted', () => {
        ...
        expect(out.fouls).toContain('scratch');
        expect(out.state.ballInHand).toBe(true);
        expect(out.state.balls.find((b) => b.number === 0)!.pocketed).toBe(
            false,
        );
    });
});
...
describe('rules  the 8 ball', () => {
    it('legally pocketing the 8 after clearing your group wins', () => {
        ...
        const pre = makeState({
            tableOpen: false,
            groups: { A: 'solids', B: 'stripes' },
            balls: buildBoard([0, 8, 9, 10, 11, 12, 13, 14, 15]),
        });
    ...
    it('pocketing the 8 early loses the game', () => {
        const pre = makeState({
            tableOpen: false,
            groups: { A: 'solids', B: 'stripes' },
            balls: buildBoard([0, 1, 8, 9, 10, 11, 12, 13, 14, 15]),
        });
    ...
    it('hitting a frozen ball with nothing else reaching a rail is a foul', () => {
        ...
        const out = resolveShot(
            ...
            makeResult({
                balls: board,
                firstContactNumber: 1,
                events: [contact(1, 0.2), rail(1, 0.22)],
            }),
            table,
        );
        expect(out.fouls).toContain('no-rail');
    ...
    it('a different (non-frozen) ball reaching a rail satisfies the requirement', () => {
        ...
        const out = resolveShot(
            ...
            makeResult({
                balls: board,
                firstContactNumber: 1,
                events: [contact(1, 0.2), rail(2, 0.3)],
            }),
            table,
        );
    ...
    it('a frozen ball that banks away and returns to a rail counts', () => {
        ...
        const out = resolveShot(
            ...
            makeResult({
                balls: board,
                firstContactNumber: 1,
                events: [contact(1, 0.2), rail(1, 0.6)],
            }),
            table,
        );
        expect(out.fouls).not.toContain('no-rail');
...
describe('rules  strict called shots (§9/§257)', () => {
    const pocket = (ball: number, idx: number): ShotEvent => ({
        type: 'pocket',
        t: 0.5,
        ballNumber: ball,
        pocketIndex: idx,
    ...
    it('a legal ball in the called pocket continues the turn', () => {
        const pre = makeState({
            tableOpen: false,
            groups: { A: 'solids', B: 'stripes' },
            balls: buildBoard([0, 1, 2, 8, 9, 10, 11, 12, 13, 14, 15]),
        });
