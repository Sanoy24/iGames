...
export class PresenceService {
    ...
    private async computeBotCounts(): Promise<BotPresenceCounts> {
        const bot =
            "JSON_EXTRACT(u.productMetadata,'$.botPolicy') IS NOT NULL AND JSON_EXTRACT(u.productMetadata,'$.botPolicy.active') = true";
        ...
        const scalar = async (sql: string): Promise<number> => {
            try {
                const rows: Array<{ c: number | string }> =
                    await this.dataSource.query(sql);
                return Number(rows[0]?.c ?? 0);
            } catch (err) {
                this.logger.debug(
                    `bot count query failed: ${err instanceof Error ? err.message : err}`,
                );
                return 0;
            }
        };
