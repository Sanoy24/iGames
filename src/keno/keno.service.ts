...
import {
    KenoTicket,
    KenoTicketStatus,
    KenoSettlementStatus,
} from './entities/keno-ticket.entity';
...
export class KenoService {
    ...
    async getActiveConfig(): Promise<KenoConfig> {
        const config = await this.kenoConfigRepository.findOneBy({
            status: 'active',
        });
        if (!config) {
            throw new NotFoundException(
                'No active Keno config. Create one via POST /admin/keno/configs before tickets can be sold.',
            );
        }
        return config;
    }
    ...
    async createConfig(dto: CreateKenoConfigDto): Promise<KenoConfig> {
        ...
        const autoScheduleIntervalSeconds =
            this.normalizeAutoScheduleIntervalSeconds(dto);
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            const config = configRepo.create({
                ...
                autoScheduleIntervalMinutes: Math.floor(
                    autoScheduleIntervalSeconds / 60,
                ),
    ...
    async purchaseTicket(input: {
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            const existingTicket = await ticketRepo.findOneBy({
                userId: input.userId,
                idempotencyKey: input.idempotencyKey,
            });
            ...
            this.kenoRulesService.validateSelectedNumbers(
    ...
    async updateTicketNumbers(input: {
        ...
        return this.dataSource.transaction(async (manager) => {
            const ticketRepo = manager.getRepository(KenoTicket);
            const ticket = await ticketRepo.findOneBy({
                id: input.ticketId,
                userId: input.userId,
            });
            ...
            const draw = await manager
                ...
                .findOneBy({ id: ticket.drawId });
            if (!draw || draw.status !== 'open') {
                throw new ConflictException(
                    'Draw is locked  numbers can no longer be changed',
                );
            }
            ...
            if (
                input.selectedNumbers.length !== ticket.selectedNumbers.length
            ) {
                throw new BadRequestException(
                    'Cannot change the number of spots after paying',
                );
            }

            ticket.selectedNumbers = [...input.selectedNumbers].sort(
                (left, right) => left - right,
            );
            const saved = await ticketRepo.save(ticket);
            return this.toTicketResponse(saved);
    ...
    async findStuckDraws(thresholdMinutes = 10): Promise<string[]> {
        ...
        const draws = await this.kenoDrawRepository
            .createQueryBuilder('draw')
            .where('draw.status IN (:...statuses)', {
        ...
        return this.dataSource.transaction(async (manager) => {
            ...
            if (winChancePct >= 100 && tickets.length > 0) {
                ...
                const paytableForSpots = config.paytable
                    .filter(
                        (e) => e.spots === spotCount && e.payoutMultiplier > 0,
                    )
                    .sort((a, b) => a.matches - b.matches);

                const minMatchesNeeded =
                    paytableForSpots.length > 0
            ...
            const autoScheduleIntervalMs =
                this.getAutoScheduleIntervalMs(config);
            if (autoScheduleIntervalMs > 0) {
                const existingOpen = await drawRepo.findOneBy({
                    status: 'open',
                });
                if (!existingOpen) {
    ...
    async scheduleDraw(scheduledAt?: Date): Promise<KenoDrawResponse> {
        ...
        const date =
            ...
            new Date(
                Date.now() +
                    (autoScheduleIntervalMs > 0
                        ? autoScheduleIntervalMs
                        : DEFAULT_AUTO_SCHEDULE_INTERVAL_SECONDS * 1000),
            );
    ...
    async cancelDraw(drawId: string): Promise<KenoDrawResponse> {
        return this.dataSource.transaction(async (manager) => {
            ...
            if (draw.status === 'settled') {
                throw new ConflictException(
                    'Settled Keno draws cannot be cancelled',
                );
            }
    ...
    private async settleDrawTickets(
        ...
        for (const ticket of tickets) {
            ...
            const cappedOut =
                maxWinners > 0 && rawPayout > 0 && winners >= maxWinners
                    ? 0
                    : rawPayout;
    ...
    private async getOpenDrawById(
        ...
        const draw = await manager
            .getRepository(KenoDraw)
            .findOneBy({ id: drawId, status: 'open' });
        if (!draw) {
    ...
    private async getOrCreateOpenDraw(
        ...
        const drawRepo = manager.getRepository(KenoDraw);
        const existingDraw = await drawRepo.findOneBy({
            status: 'open',
            configVersion: config.version,
        });
        ...
        const draw = drawRepo.create({
            ...
            scheduledAt: new Date(
                Date.now() + this.getAutoScheduleIntervalMs(config),
            ),
            drawnNumbers: [],
            settlementSummary: {},
        });
    ...
    getAutoScheduleIntervalMs(
        config: Pick<
            KenoConfig,
            'autoScheduleIntervalMinutes' | 'autoScheduleIntervalSeconds'
        >,
    ): number {
    ...
    private normalizeAutoScheduleIntervalSeconds(
        dto: CreateKenoConfigDto,
    ): number {
    ...
    private validateConfigDto(dto: CreateKenoConfigDto): void {
        ...
        if (numberMax <= numberMin) {
            throw new BadRequestException(
                'Keno numberMax must be greater than numberMin',
            );
        }
        ...
        if (allowedSpots.some((spotCount) => spotCount > rangeSize)) {
            throw new BadRequestException(
                'Keno allowedSpots cannot exceed number range size',
            );
        }
        ...
        for (const entry of dto.paytable) {
            if (entry.matches > entry.spots) {
                throw new BadRequestException(
                    'Keno paytable matches cannot exceed spots',
                );
            }
            if (!allowedSpots.includes(entry.spots)) {
                throw new BadRequestException(
                    'Keno paytable spots must be included in allowedSpots',
                );
            }
            const key = `${entry.spots}:${entry.matches}`;
            if (paytableKeys.has(key)) {
                throw new BadRequestException(
                    'Keno paytable entries must be unique by spots and matches',
                );
            }
    ...
    async isContinuousWinner(
        userId: string,
        manager?: EntityManager,
    ): Promise<boolean> {
        const repo = manager
            ? manager.getRepository(KenoTicket)
            : this.kenoTicketRepository;
