...
function onboardingRequirement(h: RoundHuman, o: WinControlOptions): WinRequirement {
  if (!o.onboardingEnabled) return 'neutral';
  if (h.gamesPlayed < o.onboardingBotWinGames) return 'lose';
  if (h.gamesPlayed < o.onboardingBotWinGames + o.onboardingUserWinGames) return 'win';
  return 'neutral';
}
...
export function applyRoundWinControl(
  ...
  const smallRound = o.winControlEnabled && o.realPlayers < o.houseGuaranteedBelowPlayers;
  ...
  for (const h of humans) {
    ...
    if (req === 'neutral' && o.winControlEnabled && (smallRound || o.periodicForceLose)) {
      req = 'lose';
    }
    perHuman[h.key] = req;
  }
  ...
  const markReached = (b: RoundBot) => { if (o.mode === 'B') b.reachedCenter = true; };

  const winFloor = winHumans.length ? Math.min(...winHumans.map((h) => h.coinValue)) : null;
  const loseCeil = loseHumans.length ? Math.max(...loseHumans.map((h) => h.coinValue)) : null;
  ...
  if (winFloor !== null) {
    ...
    if (loseCeil !== null && ceil > loseCeil) {
      ...
      const ordered = [...out].sort((a, b) => b.coinValue - a.coinValue).slice(0, need);
      for (const b of ordered) {
        const target = loseCeil + 1 + Math.floor(rng() * 3);
        b.coinValue = clamp(Math.min(Math.max(b.coinValue, target), ceil));
        markReached(b);
      }
    }
    ...
    ordered.forEach((b, i) => {
      ...
      const target = loseCeil + 1 + (need - i) * 4 + Math.floor(rng() * 4);
 ...
 */
export function rankParticipants(mode: WinMode, entries: RankEntry[]): RankedEntry[] {
  const total = entries.length;
  const eligibleOf = (reached: boolean) => (mode === 'B' ? reached : true);
  const rows = entries.map((e) => ({ ...e, eligible: eligibleOf(e.reachedCenter), rank: 0, tieCount: 1 }));
  rows.sort((a, b) => {
  ...
  for (const e of rows) {
    ...
    if (e.coinValue !== lastVal || e.eligible !== lastElig) {
      rank = seen; lastVal = e.coinValue; lastElig = e.eligible;
    }
    e.rank = e.eligible ? rank : total;
  }
  for (const e of rows) {
    e.tieCount = e.eligible ? rows.filter((r) => r.eligible && r.rank === e.rank).length : 1;
  }
  return rows;
}
