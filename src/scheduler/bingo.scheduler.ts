        ...
        try {
            ...
            if (!this.shuttingDown && bingoPlayable) {
                try {
                    ...
                    // Whether a room is actually old enough for its first bot
                    // cartela is decided centrally inside
                    // BingoService.reconcileBotCartelasInRoom (called below via
                    // topUpBotsForOpenRoom), not here  that method is also
                    // reached directly from a real player's purchase/refund, so
                    // gating only this loop was not enough to stop a bot pile-on
                    // triggered by one of those other callers.
