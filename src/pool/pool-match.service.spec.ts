...
function harness() {
    ...
    const matchRepo = {
        ...
        findOneBy: ({ id }: { id: string }) =>
            Promise.resolve(matches.get(id) ?? null),
    } as unknown as Repository<PoolMatch>;

    const shotRepo = {
        find: ({ where }: any) =>
            Promise.resolve(shots.filter((s) => s.matchId === where.matchId)),
    } as unknown as Repository<PoolShot>;

    const userRepo = {
        findOne: ({ where }: any) =>
            Promise.resolve({ id: where.id, displayName: `User ${where.id}` }),
    ...
    const insertShot = (row: any) => {
        if (
            shots.some(
                (s) =>
                    s.matchId === row.matchId && s.shotIndex === row.shotIndex,
            )
        ) {
    ...
    const updateMatch = (criteria: any, patch: any) => {
        ...
        const ok =
            !!m &&
            (criteria.shotCount === undefined ||
                m.shotCount === criteria.shotCount) &&
    ...
    const service = new PoolMatchService(
        matchRepo,
        shotRepo,
        userRepo,
        dataSource,
...
describe('PoolMatchService', () => {
    ...
    it('rejects a shot from a non-participant', async () => {
        ...
        await expect(
            service.submitShot(m.id, 'stranger', BREAK),
    ...
    it("rejects a shot when it is not the player's turn", async () => {
        ...
        await expect(
            service.submitShot(m.id, 'ub', BREAK),
    ...
    it('is idempotent: a duplicate shot index cannot double-apply', async () => {
        ...
        await expect(
            service.submitShot(m.id, 'ua', BREAK),
        ).rejects.toBeInstanceOf(ConflictException);
    ...
    const expireTurn = (m: PoolMatch) => {
        m.turnDeadline = new Date(Date.now() - 1000);
    };
