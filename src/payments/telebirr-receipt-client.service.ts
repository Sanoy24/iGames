...
import { ParsedTelebirrReceipt, TelebirrReceiptPackage } from './types/telebirr-receipt';
...
export class TelebirrReceiptClientService {
  ...
  private async loadReceiptViaProxy(baseUrl: string, receiptNo: string): Promise<string> {
    ...
    const response = await fetch(
      ...
      { headers: key ? { 'x-proxy-key': key } : {}, signal: AbortSignal.timeout(timeoutMs) },
    );
    if (!response.ok) {
  ...
  private extractReceiptNo(receiptNoOrUrl: string): string {
    ...
    if (!trimmed) throw new BadRequestException('Telebirr receipt number is required');
