...
export const poolApi = {
  ...
  joinQueue: (stakeMinor: number) => api.post<JoinResult>('/pool/queue', { stakeMinor }).then((r) => r.data),
  leaveQueue: () => api.delete<{ removed: boolean }>('/pool/queue').then((r) => r.data),
  queueStatus: () => api.get<{ queued: boolean; stakeMinor: number | null }>('/pool/queue').then((r) => r.data),
  getMatch: (id: string) => api.get<PoolMatchView>(`/pool/matches/${id}`).then((r) => r.data),
  submitShot: (id: string, input: ShotInput) =>
    api.post<PoolShotOutcome>(`/pool/matches/${id}/shots`, input).then((r) => r.data),
  getTournament: (id: string) => api.get<PoolTournament>(`/pool/tournaments/${id}`).then((r) => r.data),
  getTournamentMatches: (id: string) => api.get<PoolMatchView[]>(`/pool/tournaments/${id}/matches`).then((r) => r.data),
  registerTournament: (id: string) => api.post<PoolTournament>(`/pool/tournaments/${id}/register`).then((r) => r.data),
};
