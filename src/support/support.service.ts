...
export class SupportService {
    ...
    async postUserMessage(userId: string, dto: PostMessageDto) {
            ...
            if (requestType === 'refund') {
                ...
                if (!requestedAmountMinor || requestedAmountMinor < 1) {
                    throw new BadRequestException(
                        'A refund request must include a positive amount.',
                    );
                }
            }
            if (dto.relatedType && dto.relatedId) {
                await this.assertRelatedOwnership(
                    userId,
                    dto.relatedType,
                    dto.relatedId,
                );
        ...
        this.gateway.emitSupportMessage(
            ticket.id,
            this.toMessageEvent(ticket.id, message, ticket.assignedAgentId),
        );
        if (requestType) {
            this.gateway.emitSupportTicketCreated({
                ...
                priority:
                    requestType === 'refund' || requestType === 'dispute'
                        ? 'high'
                        : 'normal',
            });
        }
    ...
    async listTickets(
        ...
        callerId: string,
    ): Promise<{ items: SupportTicketResponse[]; total: number }> {
        const limit = Math.min(
            Math.max(parseInt(query.limit ?? '30', 10) || 30, 1),
            100,
        );
    ...
    async updateTicket(
        ...
        if (changes.length > 0) {
            await this.appendSystemMessage(
                ticketId,
                `Agent ${agentId} updated ticket: ${changes.join(', ')}.`,
            );
        }
    ...
    async claimTicket(
        ...
        await this.appendSystemMessage(
            ticketId,
            `Agent ${agentId} claimed the ticket.`,
        );
        return this.toTicketResponse(ticket);
    }
    ...
    private async requireRequestMessage(
        messageId: string,
        ...
        if (message.requestStatus && message.requestStatus !== 'pending') {
            throw new ConflictException(
                `Request already ${message.requestStatus}`,
            );
        }
    ...
    async approveRefundRequest(
        ...
        if (message.requestType !== 'refund') {
            throw new BadRequestException(
                'Only a refund request can be approved for a refund',
            );
        }

        const amountMinor =
            dto.amountMinor ?? message.requestedAmountMinor ?? 0;
        if (amountMinor < 1)
            throw new BadRequestException('Refund amount must be positive');
        if (
            message.requestedAmountMinor &&
            amountMinor > message.requestedAmountMinor
        ) {
            throw new BadRequestException(
                'Refund amount cannot exceed the requested amount',
            );
        }
        ...
        await this.appendSystemMessage(
            ticket.id,
            `Refund of ${amountMinor} approved by agent ${agentId} (ledger ${result.ledgerEntry.id}).`,
        );
        await this.notifications.safeCreate({
            userId: ticket.userId,
            type: 'adjustment',
    ...
    async rejectRequest(
        ...
        await this.appendSystemMessage(
            ticket.id,
            `${message.requestType} request declined by agent ${agentId}: ${dto.reason.trim()}`,
        );
        await this.notifications.safeCreate({
            userId: ticket.userId,
            type: 'system',
    ...
    private async appendMessage(
        ...
        const message = await this.messageRepo.save(
            this.messageRepo.create({
                ...
                requestStatus: input.requestType
        ...
        if (relatedType === 'withdrawal') {
            ...
            if (!w || w.userId !== userId) {
                throw new ForbiddenException(
                    'Referenced withdrawal does not belong to you',
                );
            }
            ...
            if (!d || d.userId !== userId) {
                throw new ForbiddenException(
                    'Referenced deposit does not belong to you',
                );
            }
        }
    }
