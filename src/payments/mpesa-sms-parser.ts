...
export function parseMpesaSms(input: string): ParsedMpesaSms | null {
    ...
    const codeMatch =
        text.match(
            /Transaction\s+(?:number|no\.?|id)\s*:?\s*([A-Za-z0-9]{8,14})/i,
    ...
    const purchaseMatch = text.match(
        new RegExp(
            String.raw`bought\s+.*?\bof\s+${AMOUNT}\s+for\s+${PHONE}\s+on\b`,
            'i',
        ),
    );
    const receivedMatch = text.match(
        new RegExp(
            String.raw`received\s+${AMOUNT}\s+from\s+(.+?)(?:\s+${PHONE})?\s+on\b`,
            'i',
        ),
    );
    const sentMatch = text.match(
        new RegExp(
            String.raw`(transferred|sent|paid)\s+${AMOUNT}\s+to\s+(.+?)(?:\s+${PHONE})?\s+on\b`,
            'i',
        ),
    );
    ...
    if (purchaseMatch) {
        ...
        const nonBalance = amounts.find(
            (m) => !/balance\s+is\s*$/i.test(text.slice(0, m.index)),
        );
