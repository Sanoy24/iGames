...
export function rangesOverlap(
    a: WithdrawalFeeRangeLike,
    b: WithdrawalFeeRangeLike,
): boolean {
    return (
        a.minAmountMinor <= upperBound(b) && b.minAmountMinor <= upperBound(a)
    );
}
...
export function assertNoOverlap(
    ...
    if (conflict) {
        const upper =
            conflict.maxAmountMinor === null
                ? 'and above'
                : `– ${conflict.maxAmountMinor}`;
 ...
 */
export function resolveWithdrawalFeeMinor(
    amountMinor: number,
    activeRanges: WithdrawalFeeRangeLike[],
): number {
