...
export const authApi = {
    loginWithTelegram: (initData: string) =>
        api
            .post<AuthTokenResponse>('/auth/telegram/miniapp', { initData })
            .then((r) => r.data),

    devSeedPlayer: (displayName: string, initialBalanceMinor = 100000) =>
        api
            .post<AuthTokenResponse>('/dev/seed/player', {
                displayName,
                initialBalanceMinor,
            })
            .then((r) => r.data),
    ...
    devSeedAdmin: (displayName = 'Dev Admin', initialBalanceMinor = 1000000) =>
        api
            .post<AuthTokenResponse>('/dev/seed/admin', {
                displayName,
                initialBalanceMinor,
            })
            .then((r) => r.data),
    ...
    devTopup: (userId: string, amountMinor = 100_000) =>
        api
            .post<{
                ok: boolean;
                amountMinor: number;
            }>('/dev/topup', { userId, amountMinor })
            .then((r) => r.data),

    loginWithCredentials: (phoneNumber: string, password: string) =>
        api
            .post<AuthTokenResponse>('/auth/credentials', {
                phoneNumber,
                password,
            })
            .then((r) => r.data),
    ...
    resolveAgentPhone: (initData: string) =>
        api
            .post<{
                phoneNumber: string;
                displayName: string;
            }>('/auth/agent/resolve-phone', { initData })
            .then((r) => r.data),
...
export const walletApi = {
    ...
    getLedger: (limit = 20) =>
        api
            .get<LedgerEntry[]>(`/wallet/ledger?limit=${limit}`)
            .then((r) => r.data),
    ...
    requestWithdrawal: (amountMinor: number, destinationAccount: string) =>
        api
            .post<Withdrawal>('/wallet/withdraw', {
                amountMinor,
                destinationAccount,
            })
            .then((r) => r.data),
    getWithdrawals: () =>
        api.get<Withdrawal[]>('/wallet/withdrawals').then((r) => r.data),
...
function extractTelebirrReceiptBody(
    rawText: string,
): { receiptUrl: string } | { receiptNo: string } {
    const trimmed = rawText.trim();
    // 1. Full URL anywhere in the text
    const urlMatch = trimmed.match(
        /https?:\/\/transactioninfo\.ethiotelecom\.et\/receipt\/([A-Za-z0-9_-]+)/i,
    );
...
export const paymentsApi = {
    ...
    previewTelebirrReceipt: (rawText: string) =>
        api
            .post<TelebirrPreview>(
                '/payments/telebirr/preview',
                extractTelebirrReceiptBody(rawText),
                { timeout: 45000 },
            )
            .then((r) => r.data),
    ...
    submitTelebirrReceipt: (rawText: string, receiptFileUrl?: string) =>
        api
            .post(
                '/payments/telebirr/receipts',
                { ...extractTelebirrReceiptBody(rawText), receiptFileUrl },
                { timeout: 45000 },
            )
            .then((r) => r.data),
    ...
    previewTelebirrScreenshot: (file: File) => {
        ...
        return api
            .post<
                TelebirrPreview & { receiptNo: string; fileUrl: string }
            >('/payments/telebirr/preview-screenshot', form, { timeout: 45000 })
            .then((r) => r.data);
    },
    ...
    submitMpesaSms: (sms: string, receiptFileUrl?: string) =>
        api
            .post(
                '/payments/mpesa/receipts',
                { sms: sms.trim(), receiptFileUrl },
                { timeout: 45000 },
            )
            .then((r) => r.data),
    ...
    uploadReceipt: (file: File) => {
        ...
        form.append('file', file);
        return api
            .post<{ fileUrl: string }>('/payments/receipts/upload', form)
            .then((r) => r.data);
    },

    getActiveAgent: () =>
        api
            .get<ActiveAgent | null>('/payments/active-agent')
            .then((r) => r.data),
    ...
    getConfig: () =>
        api
            .get<{ minDepositMinor: number }>('/payments/config')
            .then((r) => r.data),
};
...
export const kenoApi = {
    ...
    updateTicketNumbers: (ticketId: string, selectedNumbers: number[]) =>
        api
            .patch<KenoTicket>(`/keno/tickets/${ticketId}/numbers`, {
                selectedNumbers,
            })
            .then((r) => r.data),
    listTickets: (limit = 20) =>
        api
            .get<KenoTicket[]>(`/keno/tickets?limit=${limit}`)
            .then((r) => r.data),
    getTicket: (id: string) =>
        api.get<KenoTicket>(`/keno/tickets/${id}`).then((r) => r.data),
    listDraws: (limit = 10) =>
        api.get<KenoDraw[]>(`/keno/draws?limit=${limit}`).then((r) => r.data),
};
...
export const bingoApi = {
    listRooms: (page = 1, limit = 10) =>
        api
            .get<BingoRoomListResponse>(
                `/bingo/rooms?page=${page}&limit=${limit}`,
            )
            .then((r) => r.data),
    getCurrentRoom: () =>
        api.get<BingoRoomState | null>('/bingo/current').then((r) => r.data),
    getLobby: () =>
        api
            .get<{ enabled: boolean; rooms: BingoLobbyRoom[] }>('/bingo/lobby')
            .then((r) => r.data),
    getRoomState: (roomId: string) =>
        api
            .get<BingoRoomState>(`/bingo/rooms/${roomId}/state`)
            .then((r) => r.data),
    spectateRoom: (roomId: string) =>
        api
            .get<SpectatorCard[]>(`/bingo/rooms/${roomId}/spectate`)
            .then((r) => r.data),
    purchaseTickets: (
        roomId: string,
        count: number,
...
export const crashApi = {
    ...
    getActiveRound: () =>
        api.get<CrashRound | null>('/crash/active').then((r) => r.data),
    getRecentRounds: (limit = 20) =>
        api
            .get<CrashRound[]>(`/crash/rounds?limit=${limit}`)
            .then((r) => r.data),
    getMyBets: (limit = 20) =>
        api.get<CrashBet[]>(`/crash/bets?limit=${limit}`).then((r) => r.data),
    getMyBetsForRound: (roundId: string) =>
        api
            .get<CrashBet[]>(`/crash/rounds/${roundId}/bets`)
            .then((r) => r.data),
    placeBet: (
        roundId: string,
        stakeMinor: number,
    ...
    cashOut: (roundId: string, multiplierX100: number) =>
        api
            .post<CrashBet>(`/crash/rounds/${roundId}/cashout`, {
                multiplierX100,
            })
            .then((r) => r.data),
};
...
export const locationsApi = {
    ...
    getMine: () =>
        api.get<UserLocation | null>('/locations/me').then((r) => r.data),
...
export const agentMatchApi = {
    listAgents: () =>
        api.get<OnDutyAgentOption[]>('/agent-match/agents').then((r) => r.data),
    getMine: () =>
        api
            .get<AssignedAgentResult | null>('/agent-match/me')
            .then((r) => r.data),
    setMine: (dto: { agentId?: string; other?: boolean }) =>
        api
            .patch<AssignedAgentResult>('/agent-match/me', dto)
            .then((r) => r.data),
    attempt: (latitude: number, longitude: number) =>
        api
            .post<AssignedAgentResult | null>('/agent-match/attempt', {
                latitude,
                longitude,
            })
            .then((r) => r.data),
...
export const adminApi = {
    ...
    listWithdrawalFeeRanges: () =>
        api
            ...
            }>('/admin/withdrawal-fee-ranges')
            .then((r) => r.data),
    createWithdrawalFeeRange: (dto: {
        minAmountMinor: number;
        maxAmountMinor: number | null;
        feeMinor: number;
        active?: boolean;
        ...
        api
            .post<WithdrawalFeeRange>('/admin/withdrawal-fee-ranges', dto)
            .then((r) => r.data),
    updateWithdrawalFeeRange: (
        ...
        api
            .patch<WithdrawalFeeRange>(
                `/admin/withdrawal-fee-ranges/${id}`,
                dto,
            )
            .then((r) => r.data),
    deleteWithdrawalFeeRange: (id: string) =>
        api
            .delete<void>(`/admin/withdrawal-fee-ranges/${id}`)
            .then((r) => r.data),
    getAgentPerformance: () =>
        api
            .get<AgentPerformance[]>('/admin/agents/performance')
            .then((r) => r.data),
    /** The Master Wallet  the ONE shared system wallet every admin account operates on (see backend AdminService). */
    getHouseWallet: () =>
        api.get<Wallet>('/admin/wallet/house').then((r) => r.data),
...
export const notificationsApi = {
    ...
    markRead: (ids?: string[]) =>
        api
            .post<{
                unreadCount: number;
            }>('/notifications/read', ids && ids.length ? { ids } : {})
            .then((r) => r.data),
};
...
export const broadcastImageUrl = (
    imagePath: string | null | undefined,
): string | null => (imagePath ? `${ASSET_BASE}/uploads/${imagePath}` : null);
...
export const broadcastApi = {
    ...
    get: (id: string) =>
        api
            .get<BroadcastMessage>(`/admin/broadcasts/${id}`)
            .then((r) => r.data),
    uploadImage: (file: File) => {
        ...
        return api
            .post<{
                imageFilename: string;
                imagePath: string;
            }>('/admin/broadcasts/upload', form)
            .then((r) => r.data);
    },
    create: (dto: CreateBroadcastInput) =>
        api
            .post<BroadcastMessage>('/admin/broadcasts', dto)
            .then((r) => r.data),
    sendNow: (id: string) =>
        api
            .post<BroadcastMessage>(`/admin/broadcasts/${id}/send`)
            .then((r) => r.data),
    cancel: (id: string) =>
        api
            .post<BroadcastMessage>(`/admin/broadcasts/${id}/cancel`)
            .then((r) => r.data),
    remove: (id: string) =>
        api.delete(`/admin/broadcasts/${id}`).then((r) => r.data),
};
...
export const adminKenoApi = {
    ...
    scheduleDraw: (scheduledAt?: string) =>
        api
            .post<KenoDraw>('/admin/keno/draws', { scheduledAt })
            .then((r) => r.data),
    executeDraw: (drawId: string) =>
        api
            .post<KenoDraw>(`/admin/keno/draws/${drawId}/execute`)
            .then((r) => r.data),
    cancelDraw: (drawId: string) =>
        api
            .post<KenoDraw>(`/admin/keno/draws/${drawId}/cancel`)
            .then((r) => r.data),
};
...
export const adminBingoApi = {
    getConfig: () =>
        api.get<BingoConfig>('/admin/bingo/config').then((r) => r.data),
    getRoomDetails: (roomId: string) =>
        api
            .get<BingoRoundDetails>(`/admin/bingo/rooms/${roomId}/details`)
            .then((r) => r.data),
    ...
    updateRoomDisplay: (
        ...
        api
            .patch<BingoRoom>(`/admin/bingo/rooms/${roomId}/display`, dto)
            .then((r) => r.data),
    ...
    listRoomSlots: () =>
        api.get<BingoRoomSlot[]>('/admin/bingo/room-slots').then((r) => r.data),
    updateRoomSlot: (
        ownerId: string,
        dto: {
            label?: string | null;
        ...
        api
            .patch<
                BingoRoomSlot[]
            >(`/admin/bingo/room-slots/${encodeURIComponent(ownerId)}`, dto)
            .then((r) => r.data),
    ...
    listCustomRoomSlots: () =>
        api
            .get<BingoCustomRoomSlot[]>('/admin/bingo/room-slots/custom')
            .then((r) => r.data),
    createCustomRoomSlot: (dto: {
        ...
        api
            .post<BingoCustomRoomSlot>('/admin/bingo/room-slots/custom', dto)
            .then((r) => r.data),
    updateCustomRoomSlot: (
        ...
        api
            .patch<
                BingoCustomRoomSlot[]
            >(`/admin/bingo/room-slots/custom/${encodeURIComponent(id)}`, dto)
            .then((r) => r.data),
    deleteCustomRoomSlot: (id: string) =>
        api
            .delete<
                BingoCustomRoomSlot[]
            >(`/admin/bingo/room-slots/custom/${encodeURIComponent(id)}`)
            .then((r) => r.data),
    drawNext: (roomId: string) =>
        api
            .post<BingoRoom>(`/admin/bingo/rooms/${roomId}/draw-next`)
            .then((r) => r.data),
    cancelRoom: (roomId: string) =>
        api
            .post<BingoRoom>(`/admin/bingo/rooms/${roomId}/cancel`)
            .then((r) => r.data),
    listPatterns: () =>
        api.get<BingoPattern[]>('/admin/bingo/patterns').then((r) => r.data),
    createPattern: (dto: Partial<BingoPattern>) =>
        api
            .post<BingoPattern>('/admin/bingo/patterns', dto)
            .then((r) => r.data),
    updatePattern: (id: string, dto: Partial<BingoPattern>) =>
        api
            .patch<BingoPattern>(`/admin/bingo/patterns/${id}`, dto)
            .then((r) => r.data),
    deletePattern: (id: string) =>
        api.delete(`/admin/bingo/patterns/${id}`).then((r) => r.data),
    seedPatterns: () =>
        api
            .post<BingoPattern[]>('/admin/bingo/patterns/seed')
            .then((r) => r.data),
    /** Running rooms whose draw has stopped making progress. */
    listStalledRooms: () =>
        api
            .get<BingoStalledRoom[]>('/admin/bingo/rooms/stalled')
            .then((r) => r.data),
...
export const adminBotsApi = {
    ...
    topupBot: (id: string, amountMinor: number) =>
        api
            .post<BotUser>(`/admin/bots/${id}/topup`, { amountMinor })
            .then((r) => r.data),
    deleteBot: (id: string) =>
        api.delete(`/admin/bots/${id}`).then((r) => r.data),
    listBotActions: (params?: {
        botId?: string;
        game?: 'keno' | 'bingo' | 'crash' | 'admin';
        page?: number;
        limit?: number;
    ...
    listBotNames: () =>
        api.get<BotNameRecord[]>('/admin/bots/names').then((r) => r.data),
    ...
    importBotNames: (dto: { names: string[] }) =>
        api
            .post<BotNameRecord[]>('/admin/bots/names/import', dto)
            .then((r) => r.data),
    updateBotName: (
        id: string,
        dto: Partial<{ displayName: string; active: boolean }>,
    ) =>
        api
            .patch<BotNameRecord>(`/admin/bots/names/${id}`, dto)
            .then((r) => r.data),
...
export const adminAgentsApi = {
    ...
    setAgentOnDuty: (id: string, mode: 'auto' | 'on' | 'off') =>
        api
            .patch<User>(`/admin/agents/${id}/on-duty`, { mode })
            .then((r) => r.data),
    listActions: (limit = 100) =>
        api
            ...
            }>(`/admin/agents/actions?limit=${limit}`)
            .then((r) => r.data),
};
...
export const adminLocationsApi = {
    ...
    update: (
        ...
        api
            .patch<AdminLocation>(`/admin/locations/${id}`, dto)
            .then((r) => r.data),
    remove: (id: string) =>
        api.delete(`/admin/locations/${id}`).then((r) => r.data),
    listLocationAgents: (id: string) =>
        api
            ...
            >(`/admin/locations/${id}/agents`)
            .then((r) => r.data),
    listAgentLocations: (agentId: string) =>
        api
            .get<
                Array<PublicLocation & { isPrimary: boolean }>
            >(`/admin/agents/${agentId}/locations`)
            .then((r) => r.data),
    setAgentLocations: (
        agentId: string,
        dto: { locationIds: string[]; primaryLocationId?: string },
    ) =>
        api
            .put<
                Array<{
            ...
            >(`/admin/agents/${agentId}/locations`, dto)
            .then((r) => r.data),
};
...
export const adminWithdrawalsApi = {
    ...
    processWithdrawal: (
        ...
        api
            .post<Withdrawal>(`/admin/withdrawals/${id}/process`, {
                action,
                adminNotes,
            })
            .then((r) => r.data),
    ...
    verifyWithdrawal: (
        ...
        api
            .post<Withdrawal>(`/admin/withdrawals/${id}/verify`, {
                decision,
                notes,
            })
            .then((r) => r.data),
    ...
    completeWithAgent: (
        ...
        api
            .post<Withdrawal>(
                `/admin/withdrawals/${id}/complete-with-agent`,
                dto,
            )
            .then((r) => r.data),
    ...
    uploadReceipt: (file: File) => {
        ...
        return api
            .post<{
                fileUrl: string;
            }>('/admin/withdrawals/receipts/upload', form)
            .then((r) => r.data);
    },
};
...
export const adminSettlementsApi = {
    listAll: (page = 1, limit = 50) =>
        api
            .get<{
                ...
                page: number;
                limit: number;
            }>('/admin/settlements', { params: { page, limit } })
            .then((r) => r.data),
    listForAgent: (agentId: string, page = 1, limit = 50) =>
        api
            .get<{
                ...
                page: number;
                limit: number;
            }>(`/admin/agents/${agentId}/settlements`, { params: { page, limit } })
            .then((r) => r.data),
    create: (
        agentId: string,
        periodStart: string,
        ...
        api
            .post<AgentSettlement>(`/admin/agents/${agentId}/settlements`, {
                periodStart,
                periodEnd,
                notes,
            })
    ...
    update: (
        ...
        api
            .patch<AgentSettlement>(`/admin/settlements/${id}`, dto)
            .then((r) => r.data),
    ...
    uploadReceipt: (file: File) => {
        ...
        return api
            .post<{
                fileUrl: string;
            }>('/admin/settlements/receipts/upload', form)
            .then((r) => r.data);
    },
};
...
export const agentApi = {
    getConfig: () =>
        api
            ...
            .then((r) => r.data),
    getPerformance: () =>
        api.get<AgentSelfPerformance>('/agent/performance').then((r) => r.data),
    getDashboard: () =>
        api.get<AgentDashboardSummary>('/agent/dashboard').then((r) => r.data),
    getSettlements: (page = 1, limit = 50) =>
        api
            .get<{
                ...
                page: number;
                limit: number;
            }>('/agent/settlements', { params: { page, limit } })
            .then((r) => r.data),
    requestSettlement: () =>
        api
            .post<AgentSettlement>('/agent/settlements/request')
            .then((r) => r.data),
    getReferral: () =>
        api.get<AgentReferral>('/agent/referral').then((r) => r.data),
    getAvailableWithdrawals: () =>
        api.get<Withdrawal[]>('/agent/withdrawals').then((r) => r.data),
    getMyWithdrawals: () =>
        api.get<Withdrawal[]>('/agent/withdrawals/my').then((r) => r.data),
    getTransactions: () =>
        api
            .get<{
                ledger: LedgerEntry[];
    ...
    claimWithdrawal: (id: string) =>
        api
            .post<Withdrawal>(`/agent/withdrawals/${id}/claim`)
            .then((r) => r.data),
    releaseWithdrawal: (id: string) =>
        api
            .post<Withdrawal>(`/agent/withdrawals/${id}/release`)
            .then((r) => r.data),
    rejectWithdrawal: (id: string, remarks: string) =>
        api
            .post<Withdrawal>(`/agent/withdrawals/${id}/reject`, { remarks })
            .then((r) => r.data),
    completeWithdrawal: (
        id: string,
        ...
        api
            .post<Withdrawal>(
                `/agent/withdrawals/${id}/complete`,
                { provider, proof, receiptFileUrl, transferCompletedAt },
                { timeout: 45000 },
            )
            .then((r) => r.data),
    ...
    uploadWithdrawalReceipt: (file: File) => {
        ...
        return api
            .post<{
                fileUrl: string;
            }>('/agent/withdrawals/receipts/upload', form)
            .then((r) => r.data);
    },
    transferToUser: (
        phoneNumber: string,
        ...
        api
            .post<{
                agentWallet: Wallet;
                userWallet: Wallet;
            }>('/agent/wallet/transfer-to-user', { phoneNumber, amountMinor, idempotencyKey })
            .then((r) => r.data),
    ...
    getAreaPlayerActivity: (userId: string) =>
        api
            .get<AreaPlayerActivity>(`/agent/area/players/${userId}/activity`)
            .then((r) => r.data),
};
...
export const adminUsersApi = {
    listUsers: (
        ...
        if (filters?.hasPhone !== undefined)
            url += `&hasPhone=${filters.hasPhone}`;
        if (filters?.minBalanceMinor !== undefined)
            url += `&minBalanceMinor=${filters.minBalanceMinor}`;
        if (filters?.maxBalanceMinor !== undefined)
            url += `&maxBalanceMinor=${filters.maxBalanceMinor}`;
        return api
            .get<{
                data: User[];
                total: number;
    ...
    adjustWallet: (
        ...
        api
            .post<User>(`/admin/users/${userId}/wallet/adjust`, {
                amountMinor,
                direction,
                reason,
            })
...
export const supportAgentApi = {
    list: (filter: SupportTicketFilter = {}) => {
        ...
        if (filter.assignedAgentId)
            params.set('assignedAgentId', filter.assignedAgentId);
        ...
        return api
            .get<{
                items: SupportTicket[];
                total: number;
            }>(`/agent/support/tickets?${params.toString()}`)
            .then((r) => r.data);
    },
    get: (id: string) =>
        api
            .get<SupportConversation>(`/agent/support/tickets/${id}`)
            .then((r) => r.data),
    reply: (id: string, body: string, internal = false) =>
        api
            .post<SupportMessage>(`/agent/support/tickets/${id}/messages`, {
                body,
                internal,
            })
            .then((r) => r.data),
    update: (
        ...
        api
            .patch<SupportTicket>(`/agent/support/tickets/${id}`, dto)
            .then((r) => r.data),
    claim: (id: string) =>
        api
            .post<SupportTicket>(`/agent/support/tickets/${id}/claim`)
            .then((r) => r.data),
    // Request actions are message-level now (a request is a tagged message).
    approveRefund: (
        messageId: string,
        dto: { amountMinor?: number; note?: string },
    ) =>
        api
            .post<SupportMessage>(
                `/agent/support/messages/${messageId}/refund/approve`,
                dto,
            )
            .then((r) => r.data),
    reject: (messageId: string, reason: string) =>
        api
            .post<SupportMessage>(
                `/agent/support/messages/${messageId}/reject`,
                { reason },
            )
            .then((r) => r.data),
};
...
export const gamesApi = {
    getCatalog: () =>
        api.get<GameCatalogEntry[]>('/games/catalog').then((r) => r.data),
};
...
export const adminGamesApi = {
    ...
    update: (
        ...
        api
            .patch<GameCatalogEntry>(`/admin/games/${code}`, dto)
            .then((r) => r.data),
};
...
export const adminPoolApi = {
    getConfig: () =>
        api.get<AdminPoolConfig>('/admin/pool/config').then((r) => r.data),
    updateConfig: (dto: Partial<AdminPoolConfig>) =>
        api
            .patch<AdminPoolConfig>('/admin/pool/config', dto)
            .then((r) => r.data),
    createTournament: (name?: string) =>
        api
            .post<AdminPoolTournament>('/admin/pool/tournaments', { name })
            .then((r) => r.data),
    startTournament: (id: string) =>
        api
            .post<AdminPoolTournament>(
                `/admin/pool/tournaments/${id}/start`,
                {},
            )
            .then((r) => r.data),
};
    ...
    getDeposits: (
        ...
        api
            .get<{
                data: any[];
                total: number;
            }>('/admin/deposits', { params: { provider, page, limit, status } })
            .then((r) => r.data),
    verifyDeposit: (
        provider: 'telebirr' | 'mpesa',
        id: string,
