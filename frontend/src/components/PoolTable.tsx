...
export const PoolTable = forwardRef<PoolTableHandle, Props>(function PoolTable(
    ...
    useEffect(() => {
        ...
        if (!mustCall) {
            calledPocket.current = null;
            setCalledPocketUI(null);
        }
    }, [mustCall]);
    ...
    useEffect(() => {
        ...
        if (!anim.current && queue.current.length === 0) {
            balls.current = view.board.map((b) => ({
                ...b,
                pos: { ...b.pos },
                vel: { ...b.vel },
                spin: { ...b.spin },
    ...
    const fireShot = (p: number) => {
        ...
        if (mustCallRef.current && calledPocket.current != null)
            input.calledPocket = calledPocket.current;
    ...
    useEffect(() => {
        ...
        const px = (pt: { x: number; y: number }) =>
            geom.current.rail + (rotated ? pt.y : pt.x) * geom.current.scale;
        const py = (pt: { x: number; y: number }) =>
            geom.current.rail + (rotated ? pt.x : pt.y) * geom.current.scale;
        // World direction → screen direction (transpose swaps the axes).
        const sdir = (dx: number, dy: number) =>
            rotated ? { x: dy, y: dx } : { x: dx, y: dy };
        ...
        const layout = () => {
            ...
            const rail = Math.max(
                8,
                Math.round(Math.min(availW, availH) * 0.035),
            );
            const scale = Math.max(
                1,
                Math.min((availW - rail * 2) / tw, (availH - rail * 2) / th),
            );
        ...
        const drawTable = () => {
            ...
            g.addColorStop(0, '#1a8a63');
            g.addColorStop(0.7, '#157a58');
            g.addColorStop(1, '#0e5c42');
            ctx.fillStyle = g;
            roundRect(rail * 0.5, rail * 0.5, w - rail, h - rail, 8);
            ctx.fill();
            ctx.strokeStyle = 'rgba(0,0,0,0.28)';
            ctx.lineWidth = 3;
            roundRect(rail, rail, w - rail * 2, h - rail * 2, 4);
            ctx.stroke();
            for (const p of t.pockets) {
                const pr = t.pocketRadius * geom.current.scale * 1.05;
                ctx.beginPath();
                ctx.arc(px(p), py(p), pr, 0, 7);
                ctx.fillStyle = '#080605';
                ctx.fill();
                ctx.lineWidth = 3;
                ctx.strokeStyle = '#d9a441';
                ctx.beginPath();
                ctx.arc(px(p), py(p), pr + 1.5, 0, 7);
                ctx.stroke();
            }
        };
        ...
        const drawBall = (b: Ball, radius: number) => {
            ...
            ctx.fill();
            const base =
                b.number === 0 ? '#f4efe4' : BALL_COLORS[b.number] || '#ccc';
            ...
            grad.addColorStop(0, lighten(body, 0.5));
            grad.addColorStop(0.55, body);
            grad.addColorStop(1, darken(body, 0.32));
            ctx.fillStyle = grad;
            ctx.beginPath();
            ctx.arc(cx, cy, r, 0, 7);
            ctx.fill();
        ...
        const predict = (cue: Ball) => {
            ...
            let best = Infinity,
                hit: { kind: 'ball' | 'rail'; ball?: Ball } | null = null;
            for (const b of balls.current) {
                if (b.pocketed || b.number === 0) continue;
                const fx = cue.pos.x - b.pos.x,
                    fy = cue.pos.y - b.pos.y;
            ...
            return {
                dir,
                contact: {
                    x: cue.pos.x + dir.x * best,
                    y: cue.pos.y + dir.y * best,
                },
                hit,
            };
        };
        ...
        const drawAim = (cue: Ball) => {
            ...
            ctx.save();
            ctx.setLineDash([6, 6]);
            ctx.strokeStyle = 'rgba(244,239,228,0.6)';
            ctx.lineWidth = 1.6;
            ctx.beginPath();
            ctx.moveTo(cx, cy);
            const end = p
            ...
            if (p && p.hit.kind === 'ball' && p.hit.ball) {
                ...
                ctx.beginPath();
                ctx.arc(end.x, end.y, r, 0, 7);
                ctx.stroke();
                if (showObjectPath) {
                    const ob = p.hit.ball;
                    const odx = ob.pos.x - p.contact.x,
                        ody = ob.pos.y - p.contact.y;
                    const ol = Math.hypot(odx, ody) || 1;
                    const od = sdir(odx / ol, ody / ol);
                    ctx.strokeStyle = 'rgba(217,164,65,0.9)';
                    ctx.lineWidth = 1.8;
                    ctx.beginPath();
                    ctx.moveTo(px(ob.pos), py(ob.pos));
                    ctx.lineTo(px(ob.pos) + od.x * 52, py(ob.pos) + od.y * 52);
                    ctx.stroke();
                }
            }
            ctx.restore();
            ...
            const gap = r + 4;
            const bx = cx - sd.x * gap,
                by = cy - sd.y * gap;
            const ex = cx - sd.x * (gap + 150 + pull),
                ey = cy - sd.y * (gap + 150 + pull);
            const grd = ctx.createLinearGradient(bx, by, ex, ey);
            grd.addColorStop(0, '#e8d9b4');
            grd.addColorStop(0.06, '#caa46a');
            grd.addColorStop(1, '#6b4a24');
            ctx.strokeStyle = grd;
            ctx.lineWidth = 5;
            ctx.lineCap = 'round';
            ctx.beginPath();
        ...
        const drawPocketTargets = () => {
            ...
            for (let i = 0; i < t.pockets.length; i++) {
                ...
                ctx.strokeStyle = chosen
                    ? '#f6c945'
                    : `rgba(246,201,69,${0.35 + 0.35 * pulse})`;
                ctx.beginPath();
        ...
        const strike = (input: ShotInput) => {
            power.current = 0;
            const rec = recordShot(balls.current, input, table.current, {
        ...
        const playEvent = (e: ShotEvent) => {
            if (e.type === 'ball-ball') ballClick(1);
                ...
                pocketDrop();
                if (e.pocketIndex != null)
                    rings.current.push({
                        pocketIndex: e.pocketIndex,
        ...
        const drawFrame = (now: number) => {
            ...
            if (!anim.current && !preShot.current && queue.current.length > 0) {
                ...
                if (preroll) {
                    ...
                    if (input.cuePos && c) {
                        c.pos.x = input.cuePos.x;
                        c.pos.y = input.cuePos.y;
                    }
                    strike(input);
                }
            }
            ...
            if (anim.current) {
                ...
                const done =
                    ...
                    a.time >= total;
                const clamped = Number.isFinite(total)
                    ? Math.min(a.time, total)
                    : 0;
                ...
                const lo = Math.min(
                    Math.max(0, Math.floor(fp)),
                    a.frames.length - 1,
                );
                const hi = Math.min(lo + 1, a.frames.length - 1);
                const frac = fp - lo;
                const f0 = a.frames[lo],
                    f1 = a.frames[hi];
                if (f0 && f1) {
                    balls.current.forEach((b, idx) => {
                        ...
                        if (!Number.isFinite(nx) || !Number.isFinite(ny))
                            return;
                        a.vel[idx] = {
                            x: (p1.x - p0.x) / a.frameDt,
                            y: (p1.y - p0.y) / a.frameDt,
                        };
                        b.pos.x = nx;
                        b.pos.y = ny;
                        b.pocketed = p1.pocketed;
                    });
                }
                // Fire sounds for events reached by the current playback time.
                while (
                    a.firedIdx < a.events.length &&
                    a.events[a.firedIdx].t <= a.time
                ) {
                    playEvent(a.events[a.firedIdx]);
                    a.firedIdx++;
                }
                if (done) {
                    if (a.final)
                        balls.current = a.final.map((b) => ({
                            ...b,
                            pos: { ...b.pos },
                            vel: { ...b.vel },
                            spin: { ...b.spin },
                    ...
                    if (queue.current.length === 0) {
                        balls.current = viewRef.current.board.map((b) => ({
                            ...b,
                            pos: { ...b.pos },
                            vel: { ...b.vel },
                            spin: { ...b.spin },
            ...
            if (preShot.current && cue && !cue.pocketed) {
                ...
                if (ps.input.cuePos) {
                    ...
                    cue.pos.x =
                        ps.from.x + (ps.input.cuePos.x - ps.from.x) * placeProg;
                    cue.pos.y =
                        ps.from.y + (ps.input.cuePos.y - ps.from.y) * placeProg;
                    if (placeProg < 1) drawHand(cue);
                    pullStart = PLACE_FRAC;
                }
                // Then: pull the cue back proportional to power.
                const pullProg = Math.max(
                    0,
                    Math.min(1, (prog - pullStart) / (1 - pullStart)),
                );
            ...
            if (a) {
                ...
                balls.current.forEach((b, idx) => {
                    ...
                    ctx.lineWidth = r * 1.5;
                    ctx.beginPath();
                    ctx.moveTo(cx - sd.x * len, cy - sd.y * len);
                    ctx.lineTo(cx, cy);
                    ctx.stroke();
                });
                ctx.restore();
        ...
        const render = (now: number) => {
            try {
                ...
                if (!errorLogged) {
                    errorLogged = true;
                    console.error(
                        '[pool] render frame failed  recovering to authoritative board',
                        e,
                    );
                }
                anim.current = null;
                ...
                try {
                    ...
                    /* board unavailable  keep the last good frame */
                }
            }
        ...
        const updatePlace = (e: PointerEvent) => {
            ...
            const clash = balls.current.some(
                (b) =>
                    b.number !== 0 &&
                    !b.pocketed &&
                    Math.hypot(b.pos.x - cx, b.pos.y - cy) < 2 * R,
            );
            if (clash) return;
            cue.pos.x = cx;
            cue.pos.y = cy;
        ...
        const onDown = (e: PointerEvent) => {
            ...
            if (bihActive.current && cue && !cue.pocketed) {
                ...
                const w = toWorld(p.x, p.y);
                if (
                    Math.hypot(w.x - cue.pos.x, w.y - cue.pos.y) <=
                    2.4 * table.current.ballRadius
                ) {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            g.addColorStop(0.6, '#efe9dc');
            g.addColorStop(1, '#c9c1b0');
            ctx.fillStyle = g;
            ctx.beginPath();
            ctx.arc(cx, cy, R, 0, 7);
            ctx.fill();
            ...
            ctx.beginPath();
            ctx.arc(
                cx + spin.current.side * R * 0.82,
                cy - spin.current.vertical * R * 0.82,
                7,
                0,
                7,
            );
        ...
        const set = (e: PointerEvent) => {
            ...
            const cx = S / 2,
                ...
                R = S / 2 - 4;
            let sx =
                (((e.clientX - rect.left) / rect.width) * S - cx) / (R * 0.82);
            let sy =
                -(((e.clientY - rect.top) / rect.height) * S - cy) / (R * 0.82);
        ...
        >
            ...
            <div
                ...
                style={{
                    ...
                    background:
                        'linear-gradient(180deg,#5c3a22,#3b2416 12%,#3b2416 88%,#200f07)',
                    borderRadius: 16,
                    padding: 4,
                    boxShadow: '0 20px 50px -24px rgba(0,0,0,.9)',
                }}
            >
                <canvas
                    ref={cv}
            ...
            >
                ...
                <span
                    style={{
                        ...
                        color: needsCall ? '#f6c945' : 'var(--text-muted)',
                        fontWeight: 700,
                        textAlign: 'center',
                    }}
                >
                ...
                <div
                    ...
                    style={{
                        ...
                        flex: 1,
                        minHeight: 120,
                        width: isLandscape ? 46 : 38,
                        borderRadius: 10,
                        background: '#171b21',
                        border: `2px solid ${needsCall ? '#f6c945' : '#333c48'}`,
                        display: 'flex',
                    ...
                    {Array.from({ length: SEG }, (_, i) => (
                        <div
                            ...
                            style={{
                                ...
                                boxShadow:
                                    i < litSegs
                                        ? `0 0 6px ${segColor(i)}66`
                                        : 'none',
                                transition: charging.current
                                    ? 'none'
                                    : 'background 0.1s',
                            }}
                        />
                ...
                >
                    <canvas
                        ...
                        height={88}
                        style={{
                            width: isLandscape ? 74 : 58,
                            height: isLandscape ? 74 : 58,
                            borderRadius: '50%',
                            touchAction: 'none',
                            cursor: 'pointer',
                        }}
