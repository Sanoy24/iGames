...
export class LocationsService {
    ...
    async setUserLocation(
        ...
        if (wantsOther && dto.locationId) {
            throw new BadRequestException(
                'Send either locationId or other, not both',
            );
        }
        if (!wantsOther && !dto.locationId) {
            throw new BadRequestException(
                'Send a locationId, or other: true to skip attribution',
            );
        }
    ...
    async updateLocation(
        ...
        if (dto.region !== undefined)
            location.region = dto.region?.trim() || null;
    ...
    async deleteLocation(id: string): Promise<{ deleted: true }> {
        ...
        const playerCount = await this.userRepository.countBy({
            locationId: id,
        });
        if (playerCount > 0) {
            throw new ConflictException(
    ...
    async setAgentLocations(
        ...
        if (
            ...
            !locationIds.includes(dto.primaryLocationId)
        ) {
            throw new BadRequestException(
                'primaryLocationId must be one of locationIds',
            );
        }

        if (locationIds.length > 0) {
            const found = await this.locationRepository.findBy(
    ...
    async listAgentLocations(
        agentId: string,
    ): Promise<Array<PublicLocation & { isPrimary: boolean }>> {
    ...
    async listLocationAgents(
        locationId: string,
    ): Promise<
        Array<{ agentId: string; displayName: string; isPrimary: boolean }>
    > {
        const rows: Array<{
            agentId: string;
            displayName: string;
