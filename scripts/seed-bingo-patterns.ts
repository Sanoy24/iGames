...
async function main(): Promise<void> {
  ...
  if (toCreate.length === 0) {
    ...
    await repo.save(
      toCreate.map((p) => repo.create({ ...p, isBuiltIn: true, enabled: true })),
    );
    console.log(`Seeded ${toCreate.length} pattern(s): ${toCreate.map((p) => p.name).join(', ')}`);
  }

  const all = await repo.find({ order: { sortOrder: 'ASC', createdAt: 'ASC' } });
  console.log(`\nPatterns now in database (${all.length}):`);
  for (const p of all) {
    console.log(`  • ${p.name} — ${p.patternType}${p.isBuiltIn ? ' [built-in]' : ''}${p.enabled ? '' : ' (disabled)'}`);
  }
...
main().catch((err) => {
  console.error('\nPattern seed failed:', err instanceof Error ? (err.stack ?? err.message) : err);
