...
export class AgentsService {
     ...
     * commission credited via `agent_receipt` ledger entries with
     * `sourceType: 'bingo_referral_commission'`  a % of the service fee (house
     * edge cut) generated by players the agent referred. There is no separate
     * room-owner commission (removed  `bingo_room_commission` is no longer
     * written by anything; any pre-existing such entries are historical and
     * excluded from this summary). Withdrawal fees are tracked separately. The
    ...
    async computeAgentEarnings(
        ...
        const [row] = await this.systemConfigRepository.query(
         ...
         COUNT(CASE WHEN sourceType = 'bingo_referral_commission' THEN 1 END) referralCommissionCount,
